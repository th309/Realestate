...
- **Coverage:** live scored counts (`propertyiq_scores_v2`, 2026-05-31) are **935 metros, 3,137 counties, 29,417 ZIPs** (high-confidence A/B subset: 865 / 3,044 / 25,502); history backfilled to 2001 (pre-2016 is momentum-only, C confidence). **Headline/marketing coverage copy MUST read `900+ metros, 3,000+ counties, 29,000+ ZIPs`** — conservative rounded floors that are true against live data and survive the monthly rescore's small region churn. Source them from the single constant **`COVERAGE_COPY`** (and `formatMarketsScored()` for the all-levels total) in `packages/frontend/lib/data/validation-claims.ts`; **never hardcode raw coverage counts** (the retired "3,150 / 34,000 / 925 / 33,000+" strings were both wrong and mutually inconsistent). Labeled denominators (e.g. "865 metros, full-formula era"; "380 metros, backtest sample") are NOT headline coverage and keep their own explicit, labeled numbers.
