...
async function importSingleGeography(
  ...
  try {
    ...
    // Step 3b: Filter by date cutoff (--recent flag)
    let recordsToUpsert = mappedRecords;
    if (sourceConfig.dateCutoff) {
      const cutoff = sourceConfig.dateCutoff;
      const beforeCount = recordsToUpsert.length;
      recordsToUpsert = recordsToUpsert.filter((r) => {
        const pd = r.period_date as string | undefined;
        const yr = r.year as number | string | undefined;
        if (pd) return pd >= cutoff;
        if (yr) return String(yr) >= cutoff.slice(0, 4);
        return true; // keep records without a date field
      });
      if (recordsToUpsert.length < beforeCount) {
        console.log(
          `  Date filter (>= ${cutoff}): ${beforeCount} → ${recordsToUpsert.length} records`,
        );
      }
    }

    console.log(
      `  Mapped ${recordsToUpsert.length} records (${result.rowsSkippedByMapping} skipped)`,
    );
    ...
    // Step 4: Batch upsert
    const upsertResult = await batchUpsert(supabase, recordsToUpsert, {
      tableName: geoConfig.tableName,
    ...
    result.latestPeriodDate = findLatestPeriodDate(recordsToUpsert);
    ...
    await logger.complete({
      recordsProcessed: recordsToUpsert.length,
...
export async function runSourceImport(
  ...
  console.log(
    `  Geographies: ${config.geographies.map((g) => g.id).join(", ")}`,
  );
