...
if ($psql) {
    ...
    $connectionString = "host=$dbHost port=$port dbname=$database user=$username sslmode=require"
    Write-Host "`nTesting with psql..." -ForegroundColor Yellow
    $result = & psql -h $dbHost -p $port -U $username -d $database -c "SELECT version();" 2>&1
