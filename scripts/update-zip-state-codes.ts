/**
 * Update state codes in zillow_zip using ZIP code prefix mapping
 * US ZIP codes have predictable state assignments based on their first 3 digits
 */
...
const ZIP_PREFIX_TO_STATE: Record<string, string> = {
  ...
  '020': 'MA', '021': 'MA', '022': 'MA', '023': 'MA', '024': 'MA', '025': 'MA', '026': 'MA', '027': 'MA', '055': 'MA',
  ...
  // New York 100-149, 005, 063 (some overlap)
...
async function updateStateCodes() {
  console.log('Updating state codes using ZIP prefix mapping...\n');

  // Paginate through all ZIP records
  const pageSize = 1000;
  let page = 0;
  let allZips: { id: string; region_name: string; state_code: string | null }[] = [];

  while (true) {
    const { data: zips, error } = await supabase
      .from('zillow_zip')
      .select('id, region_name, state_code')
      .range(page * pageSize, (page + 1) * pageSize - 1);

    if (error) {
      console.log('Error fetching ZIPs:', error.message);
      return;
    }
    ...
    console.log(`Fetched page ${page + 1}: ${zips.length} records (total: ${allZips.length})`);

    if (zips.length < pageSize) break;
    page++;
  }

  console.log(`\nTotal ZIP records found: ${allZips.length}`);

  let updated = 0;
  let notFound = 0;
  const updates: { id: string; state_code: string }[] = [];

  for (const zip of allZips) {
    const prefix = zip.region_name.substring(0, 3);
    const state = ZIP_PREFIX_TO_STATE[prefix];

    if (state && state !== zip.state_code) {
      updates.push({ id: zip.id, state_code: state });
    } else if (!state) {
      notFound++;
    }
  }
  ...
  console.log(`Prefixes not found: ${notFound}`);
  ...
  for (let i = 0; i < updates.length; i += batchSize) {
    ...
    for (const update of batch) {
      const { error: updateError } = await supabase
        .from('zillow_zip')
        .update({ state_code: update.state_code })
        .eq('id', update.id);
...
updateStateCodes().catch(console.error);
