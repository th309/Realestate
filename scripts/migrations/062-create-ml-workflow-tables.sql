-- Migration 062: Create ML Workflow Tables
--
-- This migration creates tables for the ML workflow system:
-- 1. propertyiq_ml_jobs - Background job tracking for ML pipeline steps
-- 2. propertyiq_ml_validations - Store ML validation results
-- 3. propertyiq_backtest_runs - Store backtest run metadata
-- 4. propertyiq_backtest_samples - Store sampling details

-- ============================================================================
-- 1. ML Jobs Table (for workflow step tracking)
-- ============================================================================

CREATE TABLE IF NOT EXISTS propertyiq_ml_jobs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    job_type VARCHAR(50) NOT NULL,           -- 'data-export', 'prepare-backtest-data', etc.
    config JSONB NOT NULL DEFAULT '{}',      -- Job configuration
...
COMMENT ON TABLE propertyiq_ml_jobs IS 'Tracks ML workflow pipeline job execution status';
COMMENT ON TABLE propertyiq_ml_validations IS 'Stores ML validation results comparing formula vs AutoGluon predictions';
