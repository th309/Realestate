 ...
 * Run with: npx ts-node scripts/check-ingestion-status.ts
 */
    ...
    if (runningError) {
        ...
        runningLogs.forEach((log, i) => {
            const elapsed = log.started_at
                ...
                : 0;
            console.log(`  ${i + 1}. ${log.source} / ${log.table_name}`);
            console.log(`     ├─ Status: ${log.status}`);
            console.log(`     ├─ Progress: ${log.records_success || 0} success / ${log.records_error || 0} errors`);
