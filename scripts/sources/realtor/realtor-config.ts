...
// Use the _History files (all geos) so each run can re-upsert a rolling window
// of recent months and pick up Realtor's revisions to prior months. The
// importer windows these to the incremental cutoff locally before upserting, so
// memory stays bounded even though the Zip core-History is ~770MB. (Hotness only
// ships as _History — there is no current-month hotness file.)
export const REALTOR_URLS = {
  national: {
    core: `${S3_BASE}/Core/RDC_Inventory_Core_Metrics_Country_History.csv`,
  },
  state: {
    core: `${S3_BASE}/Core/RDC_Inventory_Core_Metrics_State_History.csv`,
  },
  metro: {
    core: `${S3_BASE}/Core/RDC_Inventory_Core_Metrics_Metro_History.csv`,
    hotness: `${S3_BASE}/Hotness/RDC_Inventory_Hotness_Metrics_Metro_History.csv`,
  },
  county: {
    core: `${S3_BASE}/Core/RDC_Inventory_Core_Metrics_County_History.csv`,
    hotness: `${S3_BASE}/Hotness/RDC_Inventory_Hotness_Metrics_County_History.csv`,
  },
  zip: {
    core: `${S3_BASE}/Core/RDC_Inventory_Core_Metrics_Zip_History.csv`,
...
export const REALTOR_TABLES: Record<
  string,
  { tableName: string; conflictKeys: string[] }
> = {
  national: { tableName: "realtor_national", conflictKeys: ["period_date"] },
  state: {
    tableName: "realtor_state",
    conflictKeys: ["period_date", "state_id"],
  },
  metro: {
  ...
  county: {
    ...
    conflictKeys: ["period_date", "county_fips"],
  },
  zip: {
    tableName: "realtor_zip",
    conflictKeys: ["period_date", "postal_code"],
  },
};
 ...
 */
export function buildNationalStateConfig(
  useHistory: boolean,
): ImportSourceConfig {
  return {
    source: "realtor",
