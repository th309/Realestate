...
The rankings also include each market's PropertyIQ Score. A rent-to-price ratio without a market conditions score is an incomplete picture. A ratio of 0.65% in a declining market reads differently than the same ratio in a market where demand is outpacing supply. Both figures belong in the same analysis.
...
The 90 score reflects current supply-demand balance: inventory growth is being absorbed, the pending-to-active ratio is 0.5623, and Zillow forecasts 2.5% near-term appreciation. It does not erase the structural questions about the local economy. Both Rochester and Detroit post identical ratios at 0.57%, but represent meaningfully different risk profiles.
