...
The PropertyIQ Score is a forward-looking read on how the market is likely to perform over the next few years relative to its state — built from the demand signal of home-value momentum, days on market, and price-reduced share, not whether an average household can afford to buy. At 91, the model expects Seattle to outperform the typical Washington market. The signals behind that read:
