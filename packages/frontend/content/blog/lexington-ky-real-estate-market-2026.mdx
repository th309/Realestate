...
The Lexington KY real estate market 2026 scores 51 out of 100 on the PropertyIQ index as of February 28, 2026. That puts Lexington slightly above the state average, but the trend is pointed in the wrong direction. One year ago the score stood at 74. The market has shed 23 points in twelve months.
...
**The score in context.** A score of 51 means the model expects Lexington to perform about in line with the typical Kentucky market over the next few years. The PropertyIQ index sets 50 as the state average, so a 51 is not a distressed market. It is a market that has cooled considerably from a year of strong conditions.
...
A score of 50 represents the state average. Scores above 70 signal performance the model expects to meaningfully beat the state. Scores below 30 flag markets the model expects to underperform, with meaningful pressure on affordability, supply, or demand fundamentals.
