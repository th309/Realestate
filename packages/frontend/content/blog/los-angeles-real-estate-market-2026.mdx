...
The PropertyIQ score is a forward-looking call on performance versus the state, built on the market's demand signal — home-value momentum, days on market, and price-cut share — not a read on price direction. A score of 90 means the model expects Los Angeles to strongly outperform the typical California market over the next few years, supported by inventory tightness, sale velocity, and demand relative to supply.
