...
The PropertyIQ Score distills a market's demand signal -- home-value momentum, days on market, and the share of listings cutting price -- into a single 0-to-100 monthly score that predicts performance over roughly the next three years relative to its state. A score of 50 represents the California state average. Oakland at 99 is at the ceiling of the scale and among the top markets nationally.
...
Oakland's score of 99 signals the model expects it to strongly outperform the typical California market over the next few years, driven by a demand signal of tight inventory and fast sales -- but that same seller's-market pressure compresses the cash flow case.
...
A score of 99 means the model expects Oakland to strongly outperform its state -- and buyers are competing. Oakland at the current price level is not a market where buyers set the terms. The demand signal behind the score reflects a persistent supply-demand imbalance in favor of sellers, and the 12-month history confirms that imbalance is the norm.
