...
> {
  const sourceGeoId =
    typeof item.sourceGeoId === "string"
      ? item.sourceGeoId
  ...
  const isInherited =
    typeof item.isInherited === "boolean"
      ? item.isInherited
      : typeof item.is_inherited === "boolean"
        ? item.is_inherited
        : false;
  const isFallback =
    typeof item.isFallback === "boolean"
      ? item.isFallback
...
export async function fetchSnapshotData(
  ...
  if (config.supportedGeos && !config.supportedGeos.includes(geoLevel)) {
    console.warn(
      `[Snapshot] Metric ${metricId} not supported for ${geoLevel} (supported: ${config.supportedGeos.join(", ")})`,
    );
    return {};
  }
  ...
  if (options?.propertyType)
    params.append("propertyType", options.propertyType);
  if (options?.forecastHorizon)
    params.append("horizon", options.forecastHorizon);
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[Snapshot] API error for ${metricId} at ${geoLevel}: ${response.status} - ${fullUrl}`,
      );
      return {};
    }
    ...
    if (Object.keys(result).length === 0 && normalizedData.count > 0) {
      console.warn(
        `[Snapshot] ${metricId}: API returned ${normalizedData.count} rows but 0 matched after transform`,
      );
    }
    ...
    if (geoLevel === "zip" && Object.keys(result).length > 0) {
      ...
      console.log(
        `[Snapshot] ${metricId} at ${geoLevel}: ${Object.keys(result).length} entries, sample keys: ${sampleKeys.join(", ")}`,
      );
    }
...
async function fetchPropertyIQScoreData(
  ...
  try {
    ...
    while (hasMore) {
      ...
      if (!response.ok) {
        console.error(
          `API error for ${metricId} page ${page}: ${response.status}`,
        );
        break;
      }
      ...
      if (page >= safetyLimit) {
        console.warn(
          `Pagination safety limit reached for ${metricId} at ${geoLevel}`,
        );
        break;
      }
    }
...
function transformResponse(
  ...
  const keyField =
    config.keyField === "auto" ? getKeyFieldForGeo(geoLevel) : config.keyField;
 ...
 */
export function toHomeValues(
  data: SnapshotData,
): Record<string, number | SnapshotEntry> {
  return data;
}
