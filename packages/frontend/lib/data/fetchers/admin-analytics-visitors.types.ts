 ...
 * VISITORS TAB TYPES
 *
 * Frontend mirror of the backend's visitor-journey.types.ts. Split out of
 * admin-analytics.types.ts, which passed the 300-line limit once these were
 * added, and re-exported from it so consumers keep a single import site.
 */
 ...
 *
 * `source`, `entryType` and `landingPage` describe the FIRST session, so they
 * are acquisition facts rather than "most recent visit" facts. `converted` is
 * true when a signup completed inside the window.
 *
 * `visitorId` and `userId` are opaque internal identifiers. The API returns no
 * name, email or location alongside them — display them truncated and never
 * present them as a person's identity.
 */
export interface VisitorSummary {
  ...
  pageviews: number;
  interactions: number;
  /** Engaged time, per-session capped at 30 minutes server-side. */
  totalSeconds: number;
