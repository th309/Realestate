...
import type {
  ...
  FreshnessEconomicMetric,
  FreshnessGeoLevel,
} from "./fetchers/freshness";

function toFreshnessGeoLevel(
  geoLevel?: GeoLevel | null,
): FreshnessGeoLevel | null {
  if (!geoLevel || geoLevel === "tract") return null;
  return geoLevel;
}

function pickMostRecentRaw(
...
function getSupportedFreshnessGeos(metricId: string): FreshnessGeoLevel[] {
  ...
  if (!config) return [];
  return config.supportedGeos
    .map(toFreshnessGeoLevel)
    .filter((geo): geo is FreshnessGeoLevel => !!geo);
}
...
function pickTableDateForMetric(
  ...
  const candidates = getSupportedFreshnessGeos(metricId).map((geo) => {
    const tableGeo =
      geo === "national" && options?.nationalUsesState ? "state" : geo;
    return data.tableDates[`${prefix}_${tableGeo}`] ?? null;
  });
  return pickMostRecentRaw(candidates);
}

function resolveEconomicMetric(
  endpoint: string,
): FreshnessEconomicMetric | null {
  if (endpoint.startsWith("/api/economic/unemployment/"))
  ...
  if (endpoint.startsWith("/api/economic/cost-of-living/"))
...
function pickEconomicDate(
  ...
  const candidates = getSupportedFreshnessGeos(metricId).map(
    (geo) => byGeo?.[geo] ?? null,
  );
  return pickMostRecentRaw(candidates);
}

function pickSourceFallback(
  metricId: string,
  data: DataFreshnessResponse,
): string | null {
  const config = METRICS[metricId];
  ...
  switch (config.dataSource) {
    ...
    case "propertyiq":
      return (
        data.sourceDates.propertyiq ?? data.tableDates.propertyiq_scores ?? null
      );
    case "calculated":
      return (
        data.sourceDates.calculated ??
        data.tableDates.calculated_metrics ??
        null
      );
    case "redfin":
      return data.sourceDates.redfin ?? data.tableDates.redfin_metro ?? null;
    case "bls":
      // QCEW + CES both write to economic_metro/economic_county.period_date.
      // The backend probe orders DESC and returns whichever is freshest, so we
      // pick the most recent of the two geo levels available for this metric.
      return (
        data.sourceDates.bls ??
        pickMostRecentRaw([
          data.tableDates.economic_metro,
          data.tableDates.economic_county,
        ]) ??
        null
      );
    case "irs":
      // IRS migration writes tax_year (INT) — backend exposes as 4-digit year string
      // via sourceDates.irs or tableDates.irs_migration_county_aggregates.
      return (
        data.sourceDates.irs ??
        data.tableDates.irs_migration_county_aggregates ??
        null
      );
    case "redfin_migration":
      return (
        data.sourceDates.redfin_migration ??
        data.tableDates.redfin_migration_metro ??
        null
      );
    default:
...
export function resolveMetricFreshnessDate(
  ...
  if (economicMetric) {
    return (
      pickEconomicDate(economicMetric, data, metricId, geoLevel) ??
      pickSourceFallback(metricId, data)
    );
  }

  if (endpoint.startsWith("/api/census/")) {
    return (
      pickTableDateForMetric(metricId, data, "census", geoLevel) ??
      data.sourceDates.census_acs ??
      pickSourceFallback(metricId, data)
    );
  }
  ...
  if (endpoint.startsWith("/api/permits/")) {
    return (
      pickTableDateForMetric(metricId, data, "permits", geoLevel, {
        nationalUsesState: true,
      }) ??
      data.sourceDates.permits ??
      data.sourceDates.census ??
      null
    );
  }
  ...
  if (endpoint.startsWith("/api/zillow/forecast/")) {
    if (requestedGeo && requestedGeo !== "national") {
      return (
        data.zillowDates.forecastByGeo[requestedGeo] ??
        data.sourceDates.zillow_forecast ??
        null
      );
    }
    return (
  ...
  if (endpoint.startsWith("/api/zillow/")) {
    if (requestedGeo && requestedGeo !== "national") {
      return (
        data.zillowDates.historicalByGeo[
          requestedGeo as Exclude<FreshnessGeoLevel, "national">
        ] ??
        data.sourceDates.zillow ??
        null
      );
    }
    return (
      pickMostRecentRaw(
        getSupportedFreshnessGeos(metricId)
          ...
          .map(
            (geo) =>
              data.zillowDates.historicalByGeo[
                geo as Exclude<FreshnessGeoLevel, "national">
              ] ?? null,
          ),
      ) ??
      data.sourceDates.zillow ??
  ...
  if (endpoint.startsWith("/api/realtor/")) {
    return (
      pickTableDateForMetric(metricId, data, "realtor", geoLevel) ??
      data.sourceDates.realtor ??
      null
    );
  }
  ...
  if (endpoint.startsWith("/api/redfin/")) {
    return (
      pickTableDateForMetric(metricId, data, "redfin", geoLevel) ??
      data.sourceDates.redfin ??
      null
    );
  }
  ...
  if (endpoint.startsWith("/api/scores/")) {
    return (
      data.tableDates.propertyiq_scores ?? data.sourceDates.propertyiq ?? null
    );
  }

  if (endpoint.startsWith("/api/metrics/")) {
    if (config.dataSource === "calculated")
      return (
        data.tableDates.calculated_metrics ??
        data.sourceDates.calculated ??
        null
      );
    if (config.dataSource === "realtor")
      return data.sourceDates.realtor ?? null;
    if (config.dataSource === "redfin")
      return data.sourceDates.redfin ?? data.tableDates.redfin_metro ?? null;
    ...
    if (config.dataSource === "propertyiq")
      return data.sourceDates.propertyiq ?? null;
  }
