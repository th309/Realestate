 ...
 * - Score endpoints: 20 tests (1 score type × 20 locations)
...
describe("Graph Matrix - Snapshot Endpoints (10 tests)", () => {
  const SNAPSHOT_ENDPOINTS = [
    ...
    {
      path: "/api/realtor/listing-price/metros",
      name: "Realtor Listing Price Metros",
      minCount: 100,
    },
    {
      path: "/api/realtor/inventory/metros",
      name: "Realtor Inventory Metros",
      minCount: 100,
    },
    {
      path: "/api/realtor/dom/metros",
      name: "Realtor DOM Metros",
      minCount: 100,
    },
...
// ============================================================================
// PROPERTYIQ SCORES TESTS (1 score type × 20 metros = 20 tests)
// ============================================================================

describe("Graph Matrix - PropertyIQ Scores (20 tests)", () => {
  const SCORE_TYPE = "propertyiq";

  describe("Metro Scores", () => {
    for (const metroId of SAMPLE_METROS) {
      it(
        `PropertyIQ score for metro ${metroId}`,
        async () => {
          const response = await fetchWithTimeout(
            `${API_URL}/api/scores/metro/${metroId}`,
          );
          ...
          // PropertyIQ score should be present
          expect(data.scores[SCORE_TYPE]).toBeDefined();
          expect(data.scores[SCORE_TYPE].score).toBeGreaterThanOrEqual(0);
          expect(data.scores[SCORE_TYPE].score).toBeLessThanOrEqual(100);
        },
        API_TIMEOUT + 5000,
      );
    }
  ...
  describe("County Scores", () => {
    for (const countyId of SAMPLE_COUNTIES) {
      it(
        `PropertyIQ score for county ${countyId}`,
        async () => {
          const response = await fetchWithTimeout(
            `${API_URL}/api/scores/county/${countyId}`,
          );
  ...
  describe("ZIP Scores", () => {
    for (const zipId of SAMPLE_ZIPS) {
      it(
        `PropertyIQ score for zip ${zipId}`,
        async () => {
          const response = await fetchWithTimeout(
            `${API_URL}/api/scores/zip/${zipId}`,
          );
...
describe("Graph Matrix - Distribution Data", () => {
  it(
    "Home value distribution across 800+ metros",
    async () => {
  ...
  it(
    "Home value distribution across 3000+ counties",
    async () => {
...
describe("Graph Matrix - Comparison Charts", () => {
  it(
    "Can fetch multiple metros for side-by-side comparison",
    async () => {
  ...
  it(
    "Can fetch time series for multiple metros",
    async () => {
      const metroIds = SAMPLE_METROS.slice(0, 5);
      const results = await Promise.all(
        metroIds.map((id) => testTimeSeries("home_value", "metro", id)),
      );
  ...
  it(
    "Can compare same metric across different geography levels",
    async () => {
...
describe("Graph Matrix - Time Range Variations", () => {
  ...
  it(
    "Can fetch 36-month history (3 years)",
    async () => {
  ...
  it(
    "Can fetch 60-month history (5 years)",
    async () => {
