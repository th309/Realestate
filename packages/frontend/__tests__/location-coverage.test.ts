...
const CORE_METRIC_ENDPOINTS = {
  metro: [
    ...
    {
      endpoint: "/api/zillow/rent/metros",
      metric: "rent",
      idField: "cbsa_code",
    },
    {
      endpoint: "/api/realtor/listing-price/metros",
      metric: "listing_price",
      idField: "cbsa_code",
    },
    {
      endpoint: "/api/realtor/inventory/metros",
      metric: "inventory",
      idField: "cbsa_code",
    },
    {
      endpoint: "/api/realtor/dom/metros",
  ...
  county: [
    ...
    {
      endpoint: "/api/realtor/listing-price/counties",
      metric: "listing_price",
      idField: "county_fips",
    },
    {
      endpoint: "/api/realtor/inventory/counties",
      metric: "inventory",
      idField: "county_fips",
    },
  ],
...
async function fetchEndpoint(
  endpoint: string,
): Promise<{ status: number; data: ApiResponse; responseTime: number }> {
...
describe("Location Coverage Integration Tests", () => {
  ...
  describe("Metro Coverage", () => {
    it(
      "home value data exists for metros",
      async () => {
        ...
        expect(
          foundCount,
          `Expected all ${SAMPLE_METROS.length} metros to have data`,
        ).toBe(SAMPLE_METROS.length);
      },
      API_TIMEOUT,
    );

    it(
      "rent data exists for metros",
      async () => {
        ...
        expect(
          foundCount,
          `Expected at least ${threshold} metros with rent data`,
    ...
    it(
      "listing price data exists for metros",
      async () => {
        const { status, data } = await fetchEndpoint(
        ...
        const responseData = data.data as unknown[];
        console.log(
          `  Total metros with listing price: ${responseData.length}`,
        );
  ...
  describe("County Coverage", () => {
    it(
      "home value data exists for counties",
      async () => {
        ...
        for (const county of SAMPLE_COUNTIES) {
          const found = findLocationInData(
            responseData,
            county.id,
            "county_fips",
          );
        ...
        expect(
          foundCount,
          `Expected at least ${threshold} counties`,
        ).toBeGreaterThanOrEqual(threshold);
      },
      API_TIMEOUT,
    );
  ...
  describe("ZIP Coverage", () => {
    it(
      "home value data exists for ZIPs",
      async () => {
        ...
        expect(
          foundCount,
          `Expected at least ${threshold} ZIPs`,
        ).toBeGreaterThanOrEqual(threshold);
      },
      API_TIMEOUT,
    );
  ...
  describe("PropertyIQ Scores Coverage", () => {
    it(
      "propertyiq scores exist for metros",
      async () => {
        const { status, data } = await fetchEndpoint(
          "/api/scores/propertyiq/metros",
        );

        console.log(
          `  PropertyIQ scores: ${status} - ${Array.isArray(data.data) ? data.data.length : 0} metros`,
        );
  ...
  describe("Data Completeness Summary", () => {
    it("generates coverage report", async () => {
      ...
      report.counties = {
        total: countyRows.length,
        sampleCoverage: countyFound,
      };
      ...
      console.log("\n📊 COVERAGE REPORT:");
      console.log(
        `  Metros: ${report.metros.total} total, ${report.metros.sampleCoverage}/${SAMPLE_METROS.length} sample`,
      );
      console.log(
        `  Counties: ${report.counties.total} total, ${report.counties.sampleCoverage}/${SAMPLE_COUNTIES.length} sample`,
      );
      console.log(
        `  ZIPs: ${report.zips.total} total, ${report.zips.sampleCoverage}/${SAMPLE_ZIPS.length} sample`,
      );
