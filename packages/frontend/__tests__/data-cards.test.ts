...
if (!API_URL) throw new Error("NEXT_PUBLIC_API_URL env var required for tests");
...
const ZILLOW_ENDPOINTS = [
  ...
  "/api/zillow/national",
  "/api/zillow/cities",
  // Home Value Changes (api/realtor)
  "/api/realtor/home-value-yoy/metros",
  "/api/realtor/home-value-yoy/counties",
  "/api/realtor/home-value-yoy/states",
  "/api/realtor/home-value-yoy/zips",
  "/api/realtor/home-value-yoy/national",
  "/api/realtor/home-value-mom/metros",
  "/api/realtor/home-value-mom/counties",
  "/api/realtor/home-value-mom/states",
  "/api/realtor/home-value-mom/zips",
  "/api/realtor/home-value-mom/national",
  "/api/metrics/home-value-5yr/metros",
  "/api/metrics/home-value-5yr/counties",
  "/api/metrics/home-value-5yr/states",
  "/api/metrics/home-value-5yr/zips",
  "/api/metrics/home-value-5yr/national",
  "/api/zillow/rent/metros",
  "/api/zillow/rent/counties",
  "/api/zillow/rent/zips",
  "/api/zillow/forecast/metros",
  "/api/zillow/forecast/zips",
  // Market Activity (api/realtor)
  "/api/realtor/home-sales/metros",
  "/api/realtor/home-sales/counties",
  "/api/realtor/home-sales/states",
  "/api/realtor/home-sales/zips",
  "/api/realtor/home-sales-yoy/metros",
  "/api/realtor/home-sales-yoy/counties",
  "/api/realtor/home-sales-yoy/states",
  "/api/realtor/home-sales-yoy/zips",
  ...
  "/api/metrics/overvalued/metros", // moved from api/zillow
  ...
  describe("Zillow Endpoints", () => {
    it.each(ZILLOW_ENDPOINTS)(
      ...
      async (endpoint) => {
        ...
        console.log(
          `  ${endpoint}: ${result.status} (${result.responseTime}ms) - ${result.rowCount} rows`,
        );
  ...
  describe("Metrics Endpoints", () => {
    it.each(METRICS_ENDPOINTS)(
      ...
      async (endpoint) => {
        ...
        expect([200, 404], `${endpoint} should return 200 or 404`).toContain(
          result.status,
        );
      },
      API_TIMEOUT,
  ...
  describe("Coverage Summary", () => {
    it("generates full coverage report", async () => {
      ...
      for (const [cat, stats] of Object.entries(categories)) {
        ...
        console.log(
          `  ${cat.toUpperCase()}: ${stats.passing}/${stats.total} passing (${passRate}%), ${stats.withData} with data (${dataRate}%)`,
        );
      }

      console.log("─".repeat(60));

      const totalPassing = Object.values(categories).reduce(
        (sum, c) => sum + c.passing,
        0,
      );
      const totalWithData = Object.values(categories).reduce(
        (sum, c) => sum + c.withData,
        0,
      );
      const total = ALL_ENDPOINTS.length;

      console.log(
        `  TOTAL: ${totalPassing}/${total} passing (${((totalPassing / total) * 100).toFixed(0)}%)`,
      );
      console.log(
        `  WITH DATA: ${totalWithData}/${total} (${((totalWithData / total) * 100).toFixed(0)}%)`,
      );
