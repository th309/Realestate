 ...
 * - Free tier: PropertyIQ score teaser only
 * - Basic tier: PropertyIQ score teaser only
 * - Pro tier: Full PropertyIQ score access
 * - Enterprise tier: Full PropertyIQ score access
 *
 * Score Access Matrix:
 * | Score Type  | Free   | Basic  | Pro  | Enterprise |
 * |-------------|--------|--------|------|------------|
 * | PropertyIQ  | Teaser | Teaser | Full | Full       |
 */
...
const freeUserAuthFile = path.join(
  __dirname,
  "../fixtures/.auth/free-user.json",
);
...
async function setupMockAPI(
  ...
  await page.route("**/api/scoring/**", (route) => {
    route.fulfill({
      ...
      body: JSON.stringify(
        isProOrHigher ? MOCK_FULL_SCORE_RESPONSE : MOCK_TEASER_SCORE_RESPONSE,
      ),
    });
  });
...
test.describe("Free Tier Access", () => {
  ...
  test("free user sees full Market Health score", async ({ page }) => {
    ...
    const scoreCard = page.getByTestId("score-card-propertyiq");
    ...
    await expect(
      page.getByTestId("component-economic_foundation"),
    ).toBeVisible();
  });
  ...
  test("free user sees HomeReady teaser only", async ({ page }) => {
    ...
    const homereadyBadge = page.getByTestId("score-badge-propertyiq");
    ...
    const teaser = page.getByTestId("score-teaser-propertyiq");
  ...
  test("free user sees InvestorEdge teaser only", async ({ page }) => {
    ...
    const investoredgeBadge = page.getByTestId("score-badge-propertyiq");
  ...
  test("free user sees upgrade CTA on HomeReady teaser", async ({ page }) => {
    ...
    const upgradeCTA = page.getByTestId("upgrade-cta-propertyiq");
    await expect(upgradeCTA).toBeVisible();
    await expect(upgradeCTA).toContainText(/upgrade|pro/i);
  });

  test("free user sees upgrade CTA on InvestorEdge teaser", async ({
    page,
  }) => {
    await selectGeography(page);
    await page.getByTestId("score-badge-propertyiq").click();
  ...
  test("upgrade CTA navigates to pricing page", async ({ page }) => {
    ...
    await page.getByTestId("upgrade-cta-propertyiq").click();
  ...
  test('teaser shows "Unlock full details" message', async ({ page }) => {
    ...
    await expect(unlockMessage).toContainText(
      /unlock|full details|component breakdown/i,
    );
  });
  ...
  test("teaser shows lock icon on gated components", async ({ page }) => {
    ...
    const lockIcon = page.getByTestId("lock-icon-propertyiq");
...
test.describe("Basic Tier Access", () => {
  ...
  test("basic user sees HomeReady teaser only (same as free)", async ({
    page,
  }) => {
    await selectGeography(page);
    await page.getByTestId("score-badge-propertyiq").click();
  ...
  test("basic user sees InvestorEdge teaser only (same as free)", async ({
    page,
  }) => {
    await selectGeography(page);
    await page.getByTestId("score-badge-propertyiq").click();
...
test.describe("Pro Tier Access", () => {
  ...
  test("pro user sees full HomeReady data with components", async ({
    page,
  }) => {
    await selectGeography(page);
    ...
    const fullCard = page.getByTestId("score-card-propertyiq");
    await expect(fullCard).toBeVisible();
    await expect(page.getByTestId("score-teaser-propertyiq")).not.toBeVisible();
  ...
  test("pro user sees full InvestorEdge data with components", async ({
    page,
  }) => {
    await selectGeography(page);
  ...
  test("pro user does not see upgrade CTA", async ({ page }) => {
    ...
    await expect(page.getByTestId("upgrade-cta-propertyiq")).not.toBeVisible();
  });
  ...
  test("pro user does not see lock icons", async ({ page }) => {
    ...
    await expect(page.getByTestId("lock-icon-propertyiq")).not.toBeVisible();
  });

  test("pro user sees confidence and trend for all scores", async ({
    page,
  }) => {
    await selectGeography(page);
    ...
    await page.getByTestId("score-badge-propertyiq").click();
    await expect(
      page.getByTestId("confidence-percentage-propertyiq"),
    ).toContainText("85%");
    await expect(page.getByTestId("trend-indicator-propertyiq")).toBeVisible();
...
test.describe("Enterprise Tier Access", () => {
  ...
  test("enterprise user sees additional enterprise features", async ({
    page,
  }) => {
    await selectGeography(page);
...
test.describe("Unauthenticated User", () => {
  ...
  test("unauthenticated user sees login prompt on gated scores", async ({
    page,
  }) => {
    await page.goto("/map");
    await page.waitForLoadState("networkidle");
...
test.describe("Session Handling", () => {
  test("tier changes reflected after profile update", async ({ page }) => {
    ...
    await page.getByTestId("score-badge-propertyiq").click();
    await expect(page.getByTestId("score-teaser-propertyiq")).toBeVisible();
    await page.getByTestId("score-card-close").click();
  ...
  test("expired session shows login prompt", async ({ page }) => {
    ...
    const hasExpiredMessage = await sessionExpired
      .isVisible()
      .catch(() => false);
    const hasLoginText = await loginRedirect.isVisible().catch(() => false);

    expect(
      hasExpiredMessage || hasLoginText || page.url().includes("/login"),
    ).toBe(true);
  });
});
