...
function getBorderWidth(geoLevel: GeoLevel): number {
  switch (geoLevel) {
    case 'tract': return 0.2;
    case 'zip': return 0.3;
    case 'city': return 0.4;
    case 'county': return 0.5;
    case 'metro': return 0.8;
    default: return 1.5;
  }
}

/** Build highlight filter for searched geography. */
function calculateHighlightFilter(feature: SearchResult, geoLevel: GeoLevel): any {
 ...
 */
export function computeFillColor(value: number, min: number, max: number): string {
...
export function addMapLayers(opts: AddMapLayersOptions): void {
  const { map, geoLevel, metricFormat, minVal, maxVal, labelPointsGeojson, highlightedFeature } = opts;
...
export function addLeaderLineLayers(map: mapboxgl.Map, lineGeojson: any): void {
  ...
  if (map.getSource('leader-line-data')) {
    (map.getSource('leader-line-data') as mapboxgl.GeoJSONSource).setData(lineGeojson);
  } else {
