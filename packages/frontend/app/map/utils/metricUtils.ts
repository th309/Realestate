...
// Shared color scale - indigo to red (7 colors)
// Used by both map fills and legend display
export const COLOR_SCALE = [
  "#3949AB", // Indigo (cool - lowest, brand primary)
  "#3b82f6", // Blue
  "#22c55e", // Green
  "#eab308", // Yellow
  "#f97316", // Orange
...
export function calculateValueRange(
  ...
  const useFixedScale =
    ...
    config &&
    (!config.scaleForGeos ||
      (geoLevel != null && config.scaleForGeos.includes(geoLevel)));
  ...
  if (metricFormat === "percent") {
    ...
    const p5Index = Math.max(
      0,
      Math.floor(sorted.length * PERCENTILE_BOUNDS.MIN),
    );
    const p95Index = Math.min(
      sorted.length - 1,
      Math.floor(sorted.length * PERCENTILE_BOUNDS.MAX),
    );
    ...
    const p5Index = Math.max(
      0,
      Math.floor(positiveValues.length * PERCENTILE_BOUNDS.MIN),
    );
    const p95Index = Math.min(
      positiveValues.length - 1,
      Math.floor(positiveValues.length * PERCENTILE_BOUNDS.MAX),
    );
...
export function formatValue(
  ...
  const isPropertyIQMax =
    position === "max" &&
    metricId &&
    getMetricConfig(metricId)?.dataSource === "propertyiq";
...
export function formatTooltipValue(
  ...
  switch (metricFormat) {
    ...
    case "number":
      displayValue =
        value >= 0 ? Math.round(value).toLocaleString("en-US") : "No data";
      break;
    case "days":
      displayValue =
        value >= 0
          ? `${Math.round(value).toLocaleString("en-US")} days`
          : "No data";
      break;
    case "currency":
    default:
...
export function formatBenchmarkValue(
  ...
  switch (format) {
    case "currency":
      if (value >= CURRENCY_SCALES.MILLION)
        return `$${(value / CURRENCY_SCALES.MILLION).toFixed(1)}M`;
      if (value >= CURRENCY_SCALES.THOUSAND)
        return `$${(value / CURRENCY_SCALES.THOUSAND).toFixed(0)}K`;
