    ...
    <div className="flex-1 bg-surface-container-low rounded-2xl p-4 flex flex-col items-center border border-outline-variant overflow-hidden">
      ...
      <div className="mt-4">
        {loading ? (
          ...
          <ScoreDisplay value={currentScore} size={160} strokeWidth={10} />
        )}
      </div>
      ...
      {trend != null && !loading && (
        <div
          className={`flex items-center gap-1 mt-1 text-sm font-semibold ${trend >= 0 ? "text-green-600" : "text-red-500"}`}
        >
          {trend >= 0 ? (
