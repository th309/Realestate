...
const SubScoreDisplay: React.FC<SubScoreDisplayProps> = ({
  ...
  return (
    <div className="flex flex-col items-center flex-1 min-w-0">
      <span
        className="text-[11px] text-on-surface-variant mb-1 truncate max-w-full"
        title={label}
      >
        {metricId ? (
          <MetricTitle
      ...
      {loading ? (
        ...
        <>
          ...
          {trend.direction && (
            <div
              className={`flex items-center gap-0.5 mt-0.5 ${getTrendColor()}`}
            >
              {getTrendIcon()}
...
const DEFAULT_PROPERTYIQ_METRICS = [
  "listing_price",
  "days_on_market",
  "for_sale_inventory",
  "cap_rate",
...
const DEFAULT_HOMEREADY_METRICS = [
  "listing_price",
  "days_on_market",
  "for_sale_inventory",
];
const DEFAULT_INVESTOREDGE_METRICS = [
  "cap_rate",
  "rent_index",
  "pending_ratio",
];
const DEFAULT_MARKETHEALTH_METRICS = [
  "hotness_score",
  "inventory_yoy",
  "new_listings_yoy",
];
...
export const ScoreCards: React.FC<ScoreCardsProps> = ({
  ...
  const handleMetricsChange = useCallback(
    (scoreType: keyof MetricSelections) => (metricIds: string[]) => {
      ...
      <div className="flex flex-col gap-3">
        <M3Card className="p-6 text-center">
          ...
          <p className="text-xs text-on-surface-variant/70">
            Scores are available at the metro, county, and zip level where they
            are most predictive. Select a more specific geography to see scores.
          </p>
        </M3Card>
      </div>
    );
