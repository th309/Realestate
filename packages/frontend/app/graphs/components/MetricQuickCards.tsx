...
import type { GeoLevel } from "@/lib/data/types";
...
export function MetricQuickCards({
  primaryMarket,
  activeMetric,
  onMetricSelect,
  vertical = false,
}: MetricQuickCardsProps) {
  return (
    <div
      className={
        vertical
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5">
            {isLoading ? (
              ...
              <span
                className={`text-xs font-semibold ${isActive ? "text-on-primary-container" : "text-on-surface"}`}
              >
                {formattedValue ?? "\u2014"}
              </span>
            )}
      ...
      <div className="flex items-end justify-between gap-2">
        <div>
          {isLoading ? (
            ...
            <span
              className={`text-base font-semibold ${isActive ? "text-on-primary-container" : "text-on-surface"}`}
            >
              {formattedValue ?? "\u2014"}
            </span>
          )}
...
function Sparkline({
  ...
  const color =
    trend === "up" ? "#16a34a" : trend === "down" ? "#dc2626" : "#0891b2";
