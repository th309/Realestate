...
const STATE_CONTEXT: Record<string, string> = {
  ...
  FL: "Florida's zero state income tax, warm climate, and retirement appeal maintain strong population inflows. The state's insurance market dynamics and hurricane risk are important factors that PropertyIQ's Market Health score accounts for.",
  ...
  AZ: "Arizona's housing market experienced a dramatic boom-bust-recovery cycle, making it a key test market for PropertyIQ's scoring methodology. The state's population growth from California migration continues to drive demand.",
...
const OPENING_TEMPLATES = [
  (name: string, state: string) =>
    `The ${name} metropolitan area represents a distinct segment of ${state}'s housing landscape. PropertyIQ analyzes this market using 40+ economic, demographic, and housing indicators to generate predictive scores that forecast where home values and investment returns are heading over the next one to three years.`,
  (name: string, state: string) =>
    `Understanding the ${name} housing market requires looking beyond headline price figures. PropertyIQ's AI-powered analysis examines 40+ indicators — from inventory levels and days on market to employment trends and population flows — to predict how this ${state} metro will perform relative to other US markets.`,
  (name: string, state: string) =>
    `Whether you're considering buying a home, investing in rental property, or assessing market risk in the ${name} area, PropertyIQ provides data-driven intelligence backed by machine learning models validated against six years of actual market outcomes across ${state} and every other US state.`,
  (name: string, _state: string) =>
    `The ${name} metro area is one of 925 US metropolitan markets that PropertyIQ scores and ranks using AI. Our HomeReady, InvestorEdge, and Market Health scores distill 40+ data points into actionable intelligence — showing not just where the market stands today, but where it's likely heading.`,
  (name: string, state: string) =>
    `PropertyIQ tracks the ${name} housing market across dozens of indicators including median home values, rental rates, inventory dynamics, price-to-income ratios, and employment trends. Our scoring models — validated across four non-overlapping time windows — predict how this ${state} market compares to national benchmarks.`,
];

const MIDDLE_TEMPLATES = [
  (name: string) =>
    `PropertyIQ's three scores for the ${name} market each serve a different purpose. The HomeReady Score predicts home appreciation potential for buyers and homeowner-investors. The InvestorEdge Score measures total return potential including rental yield for buy-and-hold investors. The Market Health Score evaluates overall stability and risk factors. Together, these scores provide a comprehensive view of market opportunity and risk.`,
  (name: string) =>
    `For the ${name} market, PropertyIQ calculates three predictive scores updated monthly. HomeReady identifies appreciation potential — markets that scored highest historically returned 5.55 percentage points more per year than the lowest-scored markets. InvestorEdge evaluates total investor returns. Market Health assesses fundamental stability, flagging markets that may be overheated or economically vulnerable.`,
  (name: string) =>
    `Each month, PropertyIQ updates its AI-powered scores for ${name} using the latest available data from Zillow, Realtor.com, Redfin, the U.S. Census Bureau, FRED, BLS, and BEA. The three scores — HomeReady for buyers, InvestorEdge for investors, and Market Health for risk assessment — provide a multi-dimensional view that no single metric can offer.`,
];
...
const CLOSING_TEMPLATES = [
  ...
  (name: string) =>
    `Use PropertyIQ's interactive analytics to compare ${name} against any other US metro on 40+ metrics. Generate a free AI market report, explore historical trends on the graphs page, or see how this market ranks nationally on the scores dashboard.`,
  (name: string) =>
