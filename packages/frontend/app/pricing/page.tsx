          ...
          {PLANS.map((plan) => (
            ...
            >
              ...
              <div className="text-center mb-6">
                <h3 className={`text-lg font-semibold mb-2 ${plan.highlight ? 'text-on-primary-container' : 'text-on-surface'}`}>
                  {plan.name}
                </h3>
                <div className="flex items-baseline justify-center gap-1">
                  <span className={`text-4xl font-bold ${plan.highlight ? 'text-on-primary-container' : 'text-on-surface'}`}>
                    {plan.price}
                  </span>
                  <span className={`text-sm ${plan.highlight ? 'text-on-primary-container/70' : 'text-on-surface-variant'}`}>
                    {plan.period}
                  </span>
                </div>
                <p className={`text-sm mt-2 ${plan.highlight ? 'text-on-primary-container/80' : 'text-on-surface-variant'}`}>
                  {plan.description}
                </p>
              </div>
              ...
              <ul className="space-y-3 mb-6">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-2">
                    <Check className={`w-4 h-4 mt-0.5 shrink-0 ${plan.highlight ? 'text-primary' : 'text-primary'}`} />
                    <span className={`text-sm ${plan.highlight ? 'text-on-primary-container' : 'text-on-surface-variant'}`}>
                      {feature}
                    </span>
                  </li>
                ))}
