...
import { Calendar, Flag, Star, AlertTriangle } from 'lucide-react';
...
export function SavingsTimeline({
  ...
  const downPayment10 = medianPrice * 0.1;
  ...
  const milestones = [
    {
      pct: 5,
      amount: downPayment5,
      months: Math.ceil(downPayment5 / monthlySavings),
      label: '5% Down (FHA)',
    },
    {
      pct: 10,
      amount: downPayment10,
      months: Math.ceil(downPayment10 / monthlySavings),
      label: '10% Down',
    },
    {
      pct: 20,
      amount: downPayment20,
      months: Math.ceil(downPayment20 / monthlySavings),
  ...
  return (
    <div className="bg-surface-container rounded-2xl p-6">
              ...
              <div className="flex-1 bg-surface rounded-xl p-4">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-primary font-bold">
                    {formatTime(milestone.months)}
                  </span>
                </div>
                <p className="text-sm text-on-surface-variant">
                  Save {formatMetricValue(milestone.amount, 'currency')}
                </p>
              </div>
            </div>
          ))}
      ...
      <p className="mt-4 text-sm text-on-surface-variant text-center">
        Based on {formatMetricValue(monthlySavings, 'currency')}/month savings
        (20% of median income)
      </p>
    </div>
  );
