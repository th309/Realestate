...
export function HistoryTab() {
  ...
  const fetchData = async () => {
    ...
    try {
      const typeFilter =
        selectedScoreType !== "all" ? `?scoreType=${selectedScoreType}` : "";

      const [versionsRes, testsRes] = await Promise.all([
        fetchAPIRaw(`/api/admin/formula-versions${typeFilter}`, {
          credentials: "include",
        }),
        fetchAPIRaw(`/api/admin/ab-tests${typeFilter}`, {
          credentials: "include",
        }),
      ]);
  ...
  const activateVersion = async (version: string, scoreType: string) => {
    if (
      !confirm(
        `Activate version ${version}? This will make it the active formula.`,
      )
    )
      return;
    ...
    try {
      const response = await fetchAPIRaw(
        `/api/admin/formula-versions/activate`,
        {
          method: "POST",
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <VersionsTable versions={versions} onActivate={activateVersion} />
      ) : (
            ...
            <tr key={version.id} className="hover:bg-surface-container-low">
              ...
              <td className="px-4 py-3 text-right">
                {!version.isActive && (
                  <button
                    onClick={() =>
                      onActivate(version.version, version.scoreType)
                    }
        ...
        <tbody className="divide-y divide-outline-variant">
          {tests.map((test) => (
            <tr key={test.id} className="hover:bg-surface-container-low">
              ...
              <td className="px-4 py-3">
                <span
                  className={`text-xs px-2 py-0.5 rounded-full ${getStatusBadge(test.status)}`}
                >
                  {test.status}
                </span>
              </td>
