...
import { WorkflowStep, StepState, StepStatus, RunStepResponse } from "./types";
...
function ValidationReportView({
  ...
  metrics: ValidationReportMetrics;
}) {
  ...
  return (
    <div className="space-y-6">
      {/* Verdict Header */}
      <div
        className={`p-4 rounded-lg ${metrics.all_scores_validated ? "bg-green-900/30 border border-green-500/50" : "bg-yellow-900/30 border border-yellow-500/50"}`}
      >
        <h3
          className={`text-lg font-semibold ${metrics.all_scores_validated ? "text-green-400" : "text-yellow-400"}`}
        >
          {metrics.verdict}
        </h3>
        <p className="text-sm text-on-surface-variant mt-1">
          {metrics.verdict_detail}
        </p>
      </div>
          ...
          <table className="w-full text-xs border border-outline-variant rounded-lg overflow-hidden">
            <thead>
              <tr className="bg-surface-container-high">
                <th className="text-left py-2 px-3 font-medium text-on-surface border-b border-outline-variant">
                  Metric
                </th>
                {scoreNames.map((name) => (
                  <th
                    key={name}
                    className="text-center py-2 px-3 font-medium text-on-surface border-b border-outline-variant"
                  >
                    {name}
                  </th>
                ))}
                <th className="text-left py-2 px-3 font-medium text-on-surface border-b border-outline-variant min-w-[200px]">
                  What This Means
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-outline-variant/50">
              <tr>
                ...
                {scoreNames.map((name) => (
                  <td
                    key={name}
                    className="py-2 px-3 text-center font-mono text-green-400"
                  >
                    {summaryTable[name]?.top_quintile_excess || "—"}
                  </td>
                ))}
                <td className="py-2 px-3 text-on-surface-variant italic">
                  Properties with the highest scores beat the average market by
                  this much. Positive = good signal.
                </td>
              </tr>
              <tr>
                <td className="py-2 px-3 text-on-surface-variant">
                ...
                {scoreNames.map((name) => (
                  <td
                    key={name}
                    className="py-2 px-3 text-center font-mono text-red-400"
                  >
                    {summaryTable[name]?.bottom_quintile_excess || "—"}
                  </td>
                ))}
                <td className="py-2 px-3 text-on-surface-variant italic">
                  Properties with the lowest scores underperformed by this much.
                  Negative = score correctly warns you.
                </td>
              </tr>
              <tr className="bg-surface-container-low">
                ...
                {scoreNames.map((name) => (
                  <td
                    key={name}
                    className="py-2 px-3 text-center font-mono font-semibold text-on-surface"
                  >
                    {summaryTable[name]?.spread || "—"}
                  </td>
                ))}
                <td className="py-2 px-3 text-on-surface-variant italic font-medium">
                  The gap between winners and losers. Bigger spread = more
                  valuable score for decision-making.
                </td>
              </tr>
              <tr>
                <td className="py-2 px-3 text-on-surface-variant">
                  Top Q Beat-Market Rate
                </td>
                {scoreNames.map((name) => (
                ...
                <td className="py-2 px-3 text-on-surface-variant italic">
                  What % of high-scoring properties actually beat the market?
                  Higher = more reliable "buy" signal.
                </td>
              </tr>
              <tr>
                <td className="py-2 px-3 text-on-surface-variant">
                  Bottom Q Beat-Market Rate
                </td>
                {scoreNames.map((name) => (
                  <td key={name} className="py-2 px-3 text-center font-mono">
                ...
                <td className="py-2 px-3 text-on-surface-variant italic">
                  What % of low-scoring properties beat the market? Lower =
                  stronger "avoid" signal.
                </td>
              </tr>
              <tr>
                <td className="py-2 px-3 text-on-surface-variant">
                ...
                <td className="py-2 px-3 text-on-surface-variant italic">
                  Is this real or just luck? Below 0.05 = statistically
                  significant (less than 5% chance it's random).
                </td>
              </tr>
              <tr>
                <td className="py-2 px-3 text-on-surface-variant">
                ...
                <td className="py-2 px-3 text-on-surface-variant italic">
                  Do higher scores lead to higher returns? Range: -1 to +1.
                  Above 0.3 = meaningful relationship.
                </td>
              </tr>
            </tbody>
          </table>
      ...
      {/* Dollar Impact */}
      {metrics.dollar_impact &&
        Object.keys(metrics.dollar_impact).length > 0 && (
          <div>
            <h4 className="text-sm font-medium text-on-surface mb-3">
              Dollar Impact
            </h4>
            <div className="p-3 bg-surface-container-low rounded-lg">
              <p className="text-xs text-on-surface-variant mb-2">
                On a $
                {metrics.dollar_impact.property_value?.toLocaleString() ||
                  "500,000"}{" "}
                property over {metrics.dollar_impact.holding_period_years || 3}{" "}
                years:
              </p>
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  <p className="text-green-400 font-mono font-semibold">
                    {metrics.dollar_impact.top_quintile_gain}
                  </p>
                ...
                <div>
                  <p className="text-red-400 font-mono font-semibold">
                    {metrics.dollar_impact.bottom_quintile_loss}
                  </p>
                  <p className="text-xs text-on-surface-variant">
                    Bottom quintile
                  </p>
                ...
                <div>
                  <p className="text-on-surface font-mono font-semibold">
                    {metrics.dollar_impact.total_value_at_risk}
                  </p>
                  <p className="text-xs text-on-surface-variant">
                    Total value at risk
                  </p>
...
const WORKFLOW_STEPS: WorkflowStep[] = [
  ...
  },
  {
    ...
    description: "Analyze cached data for completeness and quality metrics",
  ...
  },
  {
    ...
    description:
      "Correlation analysis to find which scores best predict outcomes",
  ...
  },
  {
    ...
    description: "Generate statistical distributions and percentile breakdowns",
  ...
  },
  {
    id: "monthly-report",
    name: "Monthly Report",
    description: "Generate formula health report with validation metrics",
...
export default function MLWorkflowPage() {
  ...
  const [lastStepResult, setLastStepResult] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const [exportProgress, setExportProgress] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const runStepAndWait = useCallback(
    async (
      ...
      try {
        ...
        if (!data.success) {
          throw new Error(
            (data as { error?: string }).error || "Step failed to start",
          );
        }
        ...
        while (attempts < maxAttempts && isMountedRef.current) {
          ...
          try {
            // Poll job status
            const jobRes = await fetchAPIRaw(
              `/api/admin/ml-workflow/job/${jobId}`,
            );
            ...
            if (attempts % 2 === 0) {
              try {
                const progressRes = await fetchAPIRaw(
                  "/api/admin/ml-workflow/export-progress",
                );
                if (progressRes.ok) {
  ...
  const runFullWorkflow = useCallback(async () => {
    ...
    for (let i = 0; i < WORKFLOW_STEPS.length; i++) {
      ...
      if (!success) {
        setError(
          `Workflow stopped at step ${i + 1}: ${step.name} - ${stepError}`,
        );
        break;
      }
    }
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`w-2 h-2 rounded-full ${analyticsHealth ? "bg-green-500" : "bg-red-500"}`}
                />
        ...
        <div className="bg-surface-container border-b border-outline-variant">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {["metro", "county", "zip", "state"].map((geoType) => {
                ...
                return (
                  <div
                    key={geoType}
                    className={`p-3 rounded-lg ${cache?.exists ? "bg-green-500/10" : "bg-red-500/10"}`}
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`w-2 h-2 rounded-full ${cache?.exists ? "bg-green-500" : "bg-red-500"}`}
                      />
                    </div>
                    {cache?.exists ? (
                      <>
                        <p className="text-lg font-mono text-on-surface">
                          {(cache.record_count || 0).toLocaleString()}
                        </p>
                        <p className="text-xs text-on-surface-variant">
                          {cache.last_updated
            ...
            <div className="mt-3 text-xs text-on-surface-variant">
              Total:{" "}
              {(
                cacheStatus as { total_records?: number }
              ).total_records?.toLocaleString() || 0}{" "}
              records | Cache dir:{" "}
              {(cacheStatus as { cache_directory?: string }).cache_directory ||
                "Unknown"}
            </div>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {exportProgress && hasRunningStep && (
          <div className="mt-4 p-4 bg-surface-container rounded-xl">
            ...
            {(
              ...
              <div className="mb-4">
                <h3 className="text-sm font-medium text-on-surface mb-2">
                  Step Progress:{" "}
                  {
                    (exportProgress as { workflow?: { step_id?: string } })
                      .workflow?.step_id
                  }
                </h3>
                <div className="flex items-center justify-between text-xs text-on-surface-variant mb-1">
                  <span>
                    {(
                      exportProgress as {
                        workflow?: { current_substep?: string };
                      }
                    ).workflow?.current_substep || "Processing..."}
                  </span>
                  <span className="font-mono">
                    {(
                      exportProgress as {
                        workflow?: { completed_substeps?: number };
                      }
                    ).workflow?.completed_substeps || 0}{" "}
                    /{" "}
                    {(
                      exportProgress as {
                        workflow?: { total_substeps?: number };
                      }
                    ).workflow?.total_substeps || 1}
                  </span>
                </div>
                <div className="h-2 bg-surface-container-high rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${(exportProgress as { workflow?: { percent?: number } }).workflow?.percent || 0}%`,
                    }}
                  />
                </div>
              </div>
            ...
            {stepStates["data-export"]?.status === "running" &&
              (exportProgress as Record<string, { records_fetched?: number }>)
                .metro && (
                <>
                  <h3 className="text-sm font-medium text-on-surface mb-3">
                    Data Fetch by Geography
                  </h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {["metro", "county", "zip", "state"].map((geo) => {
                      const geoProgress = (
                        exportProgress as Record<
                          string,
                          {
                      ...
                      return (
                        <div
                          ...
                          className="p-3 bg-surface-container-low rounded-lg"
                        >
                          <div className="text-xs text-on-surface-variant font-medium capitalize">
                            {geo}
                          </div>
                          <div className="text-lg font-mono text-on-surface">
                            {(
                              geoProgress.records_fetched || 0
                            ).toLocaleString()}
                          </div>
        ...
        {lastStepResult && (
          <div className="mt-6 p-4 bg-surface-container rounded-xl">
            ...
            {(lastStepResult.metrics as ValidationReportMetrics | undefined)
              ?.verdict ? (
              <ValidationReportView
                metrics={lastStepResult.metrics as ValidationReportMetrics}
              />
            ) : (
        ...
        <div className="mt-8 p-6 bg-surface-container-low rounded-xl">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="text-xs text-on-surface-variant space-y-2">
                <p>
                  ...
                  <strong>Backend (Railway/NestJS)</strong> →{" "}
                  <strong>Analytics (Railway/Python)</strong>
                </p>
                <p>
                  The Analytics microservice processes the full 3.6M+ historical
                  score records using a Parquet-based caching system for fast
                  analysis.
                </p>
              </div>
            </div>
            <div>
              ...
              <div className="text-xs text-on-surface-variant space-y-2">
                <p>
                  <strong>Initial sync:</strong> Fetches full dataset using
                  1,000-record batches (Supabase row limit).
                </p>
                <p>
                  <strong>Incremental updates:</strong> Only fetches new records
                  since last cache update.
                </p>
                <p>
                  <strong>Auto-recovery:</strong> Detects incomplete caches and
                  forces full refresh.
                </p>
                <p>
                  <strong>Local Parquet:</strong> Enables millisecond reads for
                  analysis.
                </p>
              </div>
            </div>
        ...
        <div className="mt-6 p-6 bg-surface-container-low rounded-xl">
          ...
          <div className="space-y-3 text-sm text-on-surface-variant">
            <p>
              <strong>1. Data Export:</strong> Syncs the full historical dataset
              to local Parquet cache. First run fetches all records; subsequent
              runs only fetch new data (incremental).
            </p>
            <p>
              <strong>2. Prepare Backtest Data:</strong> Analyzes cached data
              quality - checks outcome availability (12m, 36m, 60m), score
              coverage, and date ranges.
            </p>
            <p>
              <strong>3. Calculate Benchmarks:</strong> Computes national
              averages and geography-specific benchmarks for measuring excess
              returns against the market.
            </p>
            <p>
              <strong>4. Feature Analysis:</strong> Runs Pearson and Spearman
              correlation analysis to find which scores (InvestorEdge,
              HomeReady) best predict actual appreciation.
            </p>
            <p>
              <strong>5. Score Explanations:</strong> Generates statistical
              distributions with mean, std, percentiles for each score type and
              geography.
            </p>
            <p>
              <strong>6. Monthly Report:</strong> Produces a comprehensive
              validation report with confidence metrics and recommendations.
            </p>
          </div>
        </div>
                ...
                <table className="w-full text-xs text-on-surface-variant">
                  ...
                  <tbody className="divide-y divide-outline-variant/50">
                    <tr>
                      ...
                      <td className="py-2 pr-4">
                        <code>/api/admin/ml-workflow/health</code>
                      </td>
                      <td className="py-2">Check analytics service health</td>
                    </tr>
                    <tr>
                      <td className="py-2 pr-4">
                        <code className="bg-primary-container text-on-primary-container px-1 rounded">
                          POST
                        </code>
                      </td>
                      <td className="py-2 pr-4">
                        <code>/api/admin/ml-workflow/run/:stepId</code>
                      </td>
                    ...
                    <tr>
                      ...
                      <td className="py-2 pr-4">
                        <code>/api/admin/ml-workflow/job/:jobId</code>
                      </td>
                ...
                <table className="w-full text-xs text-on-surface-variant">
                  ...
                  <tbody className="divide-y divide-outline-variant/50">
                    ...
                    <tr>
                      ...
                      <td className="py-2 pr-4">
                        <code>/api/v1/cache/status</code>
                      </td>
                      <td className="py-2">
                        Get cache status for all geography types
                      </td>
                    </tr>
                    <tr>
                      <td className="py-2 pr-4">
                    ...
                    <tr>
                      ...
                      <td className="py-2 pr-4">
                        <code>/api/v1/backtest/analyze</code>
                      </td>
                    ...
                    <tr>
                      ...
                      <td className="py-2 pr-4">
                        <code>/api/v1/workflow/:stepId</code>
                      </td>
                      <td className="py-2">
                        Run workflow step (data-export, calculate-benchmarks,
                        etc.)
                      </td>
                    </tr>
                  </tbody>
        ...
        <div className="mt-6 p-6 bg-surface-container-low rounded-xl">
          ...
          <div className="text-xs text-on-surface-variant space-y-2">
            <p>
              ...
              - URL of Analytics microservice (default: http://localhost:8000)
            </p>
            <p>
              <code className="bg-surface-container px-1 rounded">
                SUPABASE_URL
            ...
            <p>
              ...
              - Parquet cache directory (default: /tmp/propertyiq-cache)
            </p>
          </div>
        </div>
      </main>
