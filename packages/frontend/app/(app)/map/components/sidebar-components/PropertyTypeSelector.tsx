...
export function PropertyTypeSelector({
  ...
  return (
    <div
      className={`mt-1 ml-2 p-2 ${colors.bg} rounded-lg border ${colors.border}`}
    >
      <div className={`text-[10px] font-medium ${colors.text} mb-1.5`}>
        Property Type
      </div>
      <div className="flex gap-1">
        {options.map((option) => {
          const isDisabled =
            (option.value === "sfr" || option.value === "mfr") &&
