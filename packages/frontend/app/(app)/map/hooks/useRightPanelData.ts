 ...
 */
function generateInsight(viewMode: ViewMode, score?: number): string {
      ...
      const marketFactors: MarketFactor[] = [
        {
          ...
          value: score
            ? (score >= 70 ? "High" : score >= 40 ? "Medium" : "Low") +
              ` (${Math.round(60 + Math.random() * 35)}/100)`
            : "\u2014",
          icon: "trending_up",
        },
        {
          ...
          value:
            viewMode === "investor"
              ? `${(4 + Math.random() * 4).toFixed(1)}% Cap Rate`
              : `Medium (${Math.round(50 + Math.random() * 30)}/100)`,
          icon: "query_stats",
        },
        {
          id: "risk",
