...
export function useMapSearch({
  ...
  const handleSelectSearchResult = useCallback(
    (result: SearchResult) => {
      ...
      // No city-level metric data exists — a city search result still flies
      // the map to it and highlights the pin above, but doesn't switch
      // geoLevel (there's no "City" pill to reflect that state into).
