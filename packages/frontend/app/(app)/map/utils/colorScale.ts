...
type MapboxExpressionValue =
  | string
  | number
  | boolean
  | null
  | MapboxExpressionValue[];
...
export function getColorScale(min: number, max: number): MapboxColorExpression {
  ...
  // Calculate step boundaries for 7 discrete color buckets.
  // Uses 'step' instead of 'interpolate' so each region gets one of
  // the exact 7 legend colors — no blended intermediate shades.
  const range = max - min;
  const step = range / 7;
  ...
  return [
    ...
    ["==", ["get", "value"], null],
    NO_DATA_COLOR,
    // Step function: value < threshold → color
    [
      "step",
      ["get", "value"],
      COLOR_SCALE[0], // values < min + step
      min + step,
      COLOR_SCALE[1],
      min + step * 2,
      COLOR_SCALE[2],
