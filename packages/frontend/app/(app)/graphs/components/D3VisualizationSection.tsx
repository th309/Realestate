...
const VISUALIZATION_INSIGHTS: Record<
  D3VisualizationType,
  {
    whatItShows: string;
    howToRead: string;
...
> = {
  scatter: {
    whatItShows:
      "Each bubble represents a geographic area. The position shows how it performs on two different metrics at once.",
    howToRead:
      "Bubbles in the upper-right corner are high on both metrics. Bubble size shows a third metric, and color shows market category.",
    lookFor: [
  ...
  boxplot: {
    whatItShows:
      "Shows how values are spread out across all areas. The box shows where most values fall, and dots are unusual outliers.",
    howToRead:
      "The middle line is the median (typical value). The box covers the middle 50% of all areas. Whiskers show the range.",
    lookFor: [
  ...
  treemap: {
    whatItShows:
      "Each rectangle is a market. Bigger rectangles have higher values for the size metric. Color intensity shows the color metric.",
    howToRead:
      "Area size = one metric value, Color darkness = another metric value. Click rectangles to explore.",
    lookFor: [
  ...
  heatmap: {
    whatItShows:
      "A grid comparing multiple metrics across many areas at once. Each row is an area, each column is a metric.",
    howToRead:
      "Darker indigo = higher relative value for that metric. Colors are normalized per column so you can compare across different metric types.",
    lookFor: [
  ...
  correlation: {
    whatItShows:
      "Shows how strongly different metrics move together. Blue means they rise together, red means one rises when the other falls.",
    howToRead:
      "Numbers from -1 to +1. Near +1 (dark blue) = strong positive relationship. Near -1 (dark red) = strong negative relationship. Near 0 = no relationship.",
    lookFor: [
...
export const D3VisualizationSection: React.FC<D3VisualizationSectionProps> = ({
  ...
  const [visualizationType, setVisualizationType] =
    useState<D3VisualizationType>("scatter");
  ...
  useEffect(() => {
    ...
    if (!availableIds.includes(colorMetric)) {
      ...
      const preferredColorMetrics = [
        "hotness_score",
        "market_heat",
        "home_value_yoy",
      ];
      const defaultColor =
        preferredColorMetrics.find((m) => availableIds.includes(m)) ||
        availableIds[Math.min(3, availableIds.length - 1)];
  ...
  // Data state
  const [metricDataCache, setMetricDataCache] = useState<
  ...
  const [heatmapSettings, setHeatmapSettings] = useState({
    colorScheme: "indigo" as "indigo" | "bluePurple" | "warm" | "cool",
    showValues: true,
  ...
  const requiredMetrics = useMemo(() => {
    ...
    switch (visualizationType) {
      case "scatter":
        return [
          ...new Set(
            [xMetric, yMetric, sizeMetric, colorMetric, ...popupMetrics].filter(
              Boolean,
            ),
          ),
        ];
  ...
  const scatterData = useMemo((): ScatterDataPoint[] => {
    ...
    regions.forEach((region) => {
      ...
      const displayName =
        xData[region]?.displayName || yData[region]?.displayName || region;
  ...
  const boxPlotData = useMemo((): BoxPlotDataPoint[] => {
    ...
    const thresholds =
      CATEGORY_THRESHOLDS[colorMetric as keyof typeof CATEGORY_THRESHOLDS];
  ...
  const { heatmapData, totalHeatmapRegions } = useMemo(() => {
    ...
    if (metricsForHeatmap.length === 0)
      return { heatmapData: [] as HeatmapDataPoint[], totalHeatmapRegions: 0 };
    ...
    const regionList =
      regionLimit === "all" ? allRegions : allRegions.slice(0, regionLimit);
      ...
      return (
        <div className="flex flex-col items-center justify-center h-[400px] text-center">
          ...
          <p className="text-sm text-on-surface-variant max-w-md">
            Advanced visualizations compare data across multiple regions. Select
            State, Metro, County, or ZIP to see comparisons.
          </p>
        </div>
      );
    }
    ...
    switch (visualizationType) {
      case "scatter":
        if (scatterData.length === 0) {
          return (
            <EmptyState message="No data available for selected metrics" />
          );
        }
        return (
          <ScatterPlot
      ...
      case "correlation":
        if (correlationData.length < 2) {
          return (
            <EmptyState message="Need at least 2 metrics for correlation" />
          );
        }
        return (
          <CorrelationMatrix
            ...
            height={450}
            onCellClick={(m1, m2, corr) =>
              console.log("Correlation:", m1, m2, corr)
            }
          />
        );
      default:
        ...
        return (
          <div className="mb-4">
            <label className="text-xs font-medium text-on-surface-variant mb-2 block">
              Select Metrics to Compare ({selectedMetrics.length} selected, min
              2)
            </label>
            ...
            <p className="text-[10px] text-on-surface-variant mt-2">
              ...
              {visualizationType === "heatmap"
                ? "Shows values across regions."
                : "Shows correlation strength between metrics."}
            </p>
          </div>
        );
      default:
  ...
  const renderLegend = () => {
    ...
    if (visualizationType === "scatter" && scatterData.length > 0) {
      const categories = [
        ...new Set(
          scatterData.map((d) => d.category).filter((c): c is string => !!c),
        ),
      ];
        ...
        <div className="flex flex-wrap items-center gap-4 mt-4 pt-4 border-t border-outline-variant">
          ...
          {categories.map((cat) => (
            <div key={cat} className="flex items-center gap-1.5">
              <div
                className={`w-3 h-3 rounded-full ${colorMap[cat] || "bg-gray-500"}`}
              />
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) =>
                  setScatterSettings((s) => ({
                    ...s,
                    showRegression: e.target.checked,
                  }))
                }
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) =>
                  setScatterSettings((s) => ({
                    ...s,
                    showQuadrants: e.target.checked,
                  }))
                }
                className="rounded border-outline text-primary focus:ring-primary"
              />
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) =>
                  setScatterSettings((s) => ({
                    ...s,
                    colorByCategory: e.target.checked,
                  }))
                }
                className="rounded border-outline text-primary focus:ring-primary"
              />
        ...
        return (
          <div className="space-y-3">
            <div>
              <label className="text-sm text-on-surface-variant mb-1 block">
                Show regions{" "}
                {totalHeatmapRegions > 0 && (
                  <span className="text-xs opacity-70">
                    ({totalHeatmapRegions} available)
                  </span>
                )}
              </label>
              <select
                ...
                onChange={(e) =>
                  setHeatmapSettings((s) => ({
                    ...
                    colorScheme: e.target
                      .value as typeof heatmapSettings.colorScheme,
                  }))
                }
                className="w-full px-3 py-2 rounded-lg border border-outline bg-surface text-on-surface text-sm focus:ring-2 focus:ring-primary"
              >
                <option value="indigo">Indigo</option>
                <option value="bluePurple">Blue-Indigo</option>
  ...
  const currentVizInfo = visualizationTypes.find(
    (v) => v.type === visualizationType,
  );
  ...
  const dataCount =
    visualizationType === "scatter"
      ...
      : visualizationType === "boxplot"
        ? boxPlotData.reduce((acc, g) => acc + g.values.length, 0)
        : visualizationType === "heatmap"
          ? heatmapRegionCount
  ...
  const isHeatmapLimited =
    visualizationType === "heatmap" && heatmapRegionCount < totalHeatmapRegions;

  // Pluralize geography level correctly
  const geoLevelPlural =
    geoLevel === "metro"
      ? "metros"
      : geoLevel === "county"
        ? "counties"
    ...
    >
      ...
      {geoLevel !== "national" && !loading && dataCount > 0 && (
        <div className="text-xs text-on-surface-variant bg-surface-container/50 px-3 py-2 rounded-lg mb-4">
          <strong>Scope:</strong> This chart compares <em>all</em>{" "}
          {geoLevelPlural} nationwide, not just areas within{" "}
          {selectedArea || "your selection"}.
          {selectedArea && (
            <span className="ml-1">
              To see areas within {selectedArea}, change the geography level to
              a more specific type (e.g., County or ZIP).
            </span>
          )}
        </div>
      )}
      ...
      {!loading && dataCount > 0 && (
        <div className="text-[10px] text-on-surface-variant text-center mt-2">
          Live data from PropertyIQ • Comparing {dataCount} {geoLevelPlural}{" "}
          nationwide
        </div>
      )}
          ...
          <div className="mb-4">
            <span
              className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
                point.category === "Hot Market" ||
                point.category === "Growing" ||
                point.category === "High"
                  ? "bg-green-500/20 text-green-700 dark:text-green-400"
                  : point.category === "Cold Market" ||
                      point.category === "Declining" ||
                      point.category === "Low"
