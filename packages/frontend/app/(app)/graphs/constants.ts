...
export const DESCRIPTIONS: Record<string, string> = {
  ...
  homeowner_affordability:
    "Percentage of income needed for homeownership costs.",
  ...
  pending_ratio:
    "Ratio of pending to active listings (higher = more competitive).",
...
export const STATE_TO_CENSUS_REGION: Record<string, CensusRegion> =
  Object.fromEntries(
    Object.entries(STATE_TO_CENSUS_DIVISION).map(([st, div]) => [
      st,
      DIVISION_TO_REGION[div],
    ]),
  );
...
export function parseAllStatesFromName(name: string): string[] {
  // Strip geographic suffixes (e.g. "metro area", "county") that some APIs append
  const cleaned = name.replace(/\s+(?:metro area|county|city)\s*$/i, "");
  const match = cleaned.match(/,\s*((?:[A-Z]{2})(?:\s*-\s*[A-Z]{2})*)\s*$/);
  if (!match) return [];
 ...
 */
export function matchesAllowedStates(
  name: string,
