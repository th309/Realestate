...
export function MLValidationTab() {
    ...
    const interval = setInterval(async () => {
      try {
        const response = await fetchAPIRaw(
          `/api/admin/ml-validation/status/${jobId}`,
          {
            credentials: "include",
          },
        );
  ...
  const fetchPreviousResults = async () => {
    try {
      const response = await fetchAPIRaw(
        `/api/admin/ml-validation/results?limit=10`,
        {
          credentials: "include",
        },
      );
  ...
  const fetchValidationResult = async (validationId: string) => {
    try {
      const response = await fetchAPIRaw(
        `/api/admin/ml-validation/${validationId}`,
        {
          credentials: "include",
        },
      );
  ...
  const applySuggestions = async (
    ...
    try {
      const response = await fetchAPIRaw(
        `/api/admin/ml-validation/apply-suggestions/${result.id}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  const getGapStatus = (
    ...
    if (relativeGap > 0.1) return "review";
    return "ok";
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setConfig({
                  ...config,
                  scoreType: e.target.value as MLValidationConfig["scoreType"],
                })
              }
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setConfig({
                  ...config,
                  geographyType: e.target
                    .value as MLValidationConfig["geographyType"],
                })
              }
              className="w-full px-3 py-2 rounded-lg border border-outline bg-surface text-on-surface"
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setConfig({
                  ...config,
                  horizon: e.target.value as MLValidationConfig["horizon"],
                })
              }
              className="w-full px-3 py-2 rounded-lg border border-outline bg-surface text-on-surface"
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setConfig({
                  ...config,
                  mlPreset: e.target.value as MLValidationConfig["mlPreset"],
                })
              }
              className="w-full px-3 py-2 rounded-lg border border-outline bg-surface text-on-surface"
            >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setConfig({ ...config, trainPeriodStart: e.target.value })
              }
              className="w-full px-3 py-2 rounded-lg border border-outline bg-surface text-on-surface"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setConfig({ ...config, trainPeriodEnd: e.target.value })
              }
              className="w-full px-3 py-2 rounded-lg border border-outline bg-surface text-on-surface"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setConfig({ ...config, testPeriodStart: e.target.value })
              }
              className="w-full px-3 py-2 rounded-lg border border-outline bg-surface text-on-surface"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setConfig({ ...config, testPeriodEnd: e.target.value })
              }
              className="w-full px-3 py-2 rounded-lg border border-outline bg-surface text-on-surface"
            />
          </div>
            ...
            >
              ...
              {previousResults.map((r) => (
                <option key={r.id} value={r.id}>
                  {formatScoreType(r.scoreType)} @ {r.geographyType} @{" "}
                  {r.horizon} ({new Date(r.createdAt).toLocaleDateString()})
                </option>
              ))}
            </select>
          )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700">
                Progress: {jobStatus.progress.toFixed(0)}% | Job ID:{" "}
                {jobStatus.id.slice(0, 8)}...
              </p>
            </div>
          </div>
        </div>
      ...
      {result && (
        <>
          ...
          {result.mlLeaderboard.length > 0 && (
            <MLLeaderboard
              leaderboard={result.mlLeaderboard}
              trainingTime={result.trainingTimeSeconds}
            />
          )}
        </>
      )}
...
function PerformanceComparison({ result }: { result: MLValidationResult }) {
  const metrics = [
    {
      name: "R²",
      formula: result.formulaR2,
      ml: result.mlR2,
      format: "decimal",
    },
    {
      name: "Directional Accuracy",
      formula: result.formulaDirectionalAccuracy,
      ml: result.mlDirectionalAccuracy,
      format: "percent",
    },
    {
      name: "MAE",
      formula: result.formulaMae,
      ml: result.mlMae,
      format: "percent",
      invert: true,
    },
    ...
    },
    {
      ...
      formula: result.formulaQuintileSpread,
      ml: result.mlQuintileSpread,
      format: "percent",
    },
  ];
  ...
  return (
    <div className="bg-surface-container rounded-xl p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span
          className={`text-sm px-3 py-1 rounded-full ${statusColors[result.status]}`}
        >
          {result.status === "ok"
            ? "Healthy"
      ...
      <p className="text-sm text-on-surface-variant mb-4">
        ...
        {result.geographyType.toUpperCase()} @ {result.horizon} Horizon
      </p>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-outline-variant">
              <th className="text-left py-2 text-sm text-on-surface-variant">
                Metric
              </th>
              <th className="text-right py-2 text-sm text-on-surface-variant">
                Your Formula
              </th>
              <th className="text-right py-2 text-sm text-on-surface-variant">
                AutoGluon ML
            ...
            {metrics.map((m) => {
              ...
              return (
                <tr key={m.name} className="border-b border-outline-variant/50">
                  ...
                  <td className="py-3 text-right font-mono text-on-surface">
                    {formatValue(m.formula)}
                  </td>
                  <td className="py-3 text-right font-mono text-on-surface">
                    {formatValue(m.ml)}
                  </td>
                  <td
                    className={`py-3 text-right font-mono ${getGapColor(gap, m.invert)}`}
                  >
                    {gap >= 0 ? "+" : ""}
                    {m.format === "percent"
                      ? `${(gap * 100).toFixed(1)}%`
      ...
      <div className="mt-4 text-xs text-on-surface-variant">
        <p>
          Gap Thresholds: OK (&lt;10% relative) | Review (10-25% relative) |
          Action Required (&gt;25% relative)
        </p>
        <p className="mt-1">
          Test Samples: {result.testSamples?.toLocaleString()} | Features:{" "}
          {result.featuresUsed} | Training Time:{" "}
          {result.trainingTimeSeconds?.toFixed(0)}s
        </p>
      </div>
    </div>
  );
...
function FeatureImportanceTable({
  ...
  return (
    <div className="bg-surface-container rounded-xl p-6">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-outline-variant">
              ...
              <th className="text-left py-2 text-on-surface-variant">
                Feature
              </th>
              <th className="text-right py-2 text-on-surface-variant">
                ML Importance
              </th>
              <th className="text-right py-2 text-on-surface-variant">
                Your Weight
          ...
          <tbody>
            {featureImportance.slice(0, 15).map((fi, i) => (
              ...
              >
                ...
                <td className="py-2 text-right font-mono text-on-surface">
                  {fi.currentWeight !== null
                    ? `${(fi.currentWeight * 100).toFixed(1)}%`
                    : "\u2014"}
                </td>
                ...
                <td className="py-2">
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${statusColors[fi.status]}`}
                  >
                    {fi.status}
                  </span>
                </td>
      ...
      <div className="mt-4 text-xs text-on-surface-variant">
        <p>
          Aligned = Weight matches ML importance | Missing = Not in formula |
          Overweight/Underweight = Weight vs ML mismatch
        </p>
      </div>
    </div>
  );
                ...
                {weights.map((w) => (
                  ...
                  >
                    <td className="py-2 text-on-surface capitalize">
                      {w.component.replace("_", " ")}
                    </td>
                    <td className="py-2 text-right font-mono text-on-surface">
                      {(w.currentWeight * 100).toFixed(0)}%
                    </td>
                    <td className="py-2 text-right font-mono text-on-surface">
                      {(w.suggestedWeight * 100).toFixed(0)}%
                    </td>
                    <td
                      className={`py-2 text-right font-mono ${w.change >= 0 ? "text-green-600" : "text-red-600"}`}
                    >
                      {w.change >= 0 ? "+" : ""}
                      {(w.change * 100).toFixed(0)}%
                    </td>
              ...
              <tbody>
                {group.segments.map((seg) => (
                  ...
                  >
                    ...
                    <td className="py-2 text-right font-mono text-on-surface">
                      {seg.formulaR2.toFixed(4)}
                    </td>
                    <td className="py-2 text-right font-mono text-on-surface">
                      {seg.mlR2.toFixed(4)}
                    </td>
                    <td
                      className={`py-2 text-right font-mono ${statusColors[seg.status]}`}
                    >
                      {seg.gap >= 0 ? "+" : ""}
                      {seg.gap.toFixed(4)}
                    </td>
                    <td className="py-2 text-right text-on-surface-variant">
                      {seg.sampleSize.toLocaleString()}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <tbody>
            {leaderboard.map((entry) => (
              ...
              >
                ...
                <td className="py-2 text-on-surface font-mono">
                  {entry.model}
                </td>
                <td className="py-2 text-right font-mono text-on-surface">
                ...
                <td className="py-2 text-right text-on-surface-variant">
                  {entry.predictTime.toFixed(2)}s
                </td>
                <td className="py-2 text-right text-on-surface-variant">
                  {entry.fitTime.toFixed(0)}s
                </td>
              </tr>
            ))}
          </tbody>
