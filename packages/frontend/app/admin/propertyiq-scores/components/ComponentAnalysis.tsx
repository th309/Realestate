...
> = {
  ...
  homeready: [
    { name: "Affordability", weight: 0.3 },
    { name: "Market Timing", weight: 0.25 },
    { name: "Growth Potential", weight: 0.2 },
    { name: "Stability", weight: 0.15 },
    { name: "Liquidity", weight: 0.1 },
  ],
  investoredge: [
    { name: "Cash Flow", weight: 0.35 },
    { name: "Appreciation", weight: 0.2 },
    { name: "Rent Demand", weight: 0.2 },
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`px-2 py-0.5 text-xs font-medium rounded-full ${c.bg} ${c.text} capitalize`}
    >
      {status}
    </span>
  );
...
function ComponentBar({ component }: { component: ComponentMetrics }) {
  ...
  return (
    <div
      className={`border rounded-lg ${isWeak ? "border-error/50 bg-error/5" : "border-outline-variant"}`}
    >
      <button
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      <span className="text-on-surface-variant">
                        ...
                        <span className="font-medium">
                          {(metric.importance * 100).toFixed(1)}%
                        </span>
                      </span>
                      <span className="text-on-surface-variant">
                        Correlation:{" "}
                        <span className="font-medium">
                          {metric.correlation.toFixed(3)}
                        </span>
                      </span>
                    </div>
                  </div>
...
export function ComponentAnalysis({ geography }: ComponentAnalysisProps) {
  ...
  const generateMockData = useCallback((): ComponentAnalysisData => {
    const components =
      COMPONENT_DEFINITIONS[scoreType]?.map((def) => {
        ...
        return {
          ...
          metrics: [
            {
              name: "Primary Metric",
              importance: 0.4 + Math.random() * 0.2,
              correlation: 0.3 + Math.random() * 0.4,
            },
            {
              name: "Secondary Metric",
              importance: 0.2 + Math.random() * 0.2,
              correlation: 0.2 + Math.random() * 0.3,
            },
            {
              name: "Supporting Metric",
              importance: 0.1 + Math.random() * 0.1,
    ...
    const avgConfidence = components.reduce(
  ...
  const weakComponents =
    data?.components.filter((c) => c.confidence < 60) || [];
            ...
            {weakComponents.length > 0 && (
              <div className="mt-4 p-3 bg-error/10 border border-error/20 rounded-lg">
                <div className="flex items-center gap-2 text-error font-medium text-sm">
                  ...
                  {weakComponents.length} component
                  {weakComponents.length > 1 ? "s" : ""} with low confidence
                </div>
              ...
              {data.components.map((component, idx) => {
                ...
                return (
                  <div key={idx} className="flex items-center gap-1.5 text-sm">
                    <span
                      className={`w-3 h-3 rounded ${colors[idx % colors.length]}`}
                    ></span>
                    <span className="text-on-surface-variant">
                      {component.name}
                    </span>
