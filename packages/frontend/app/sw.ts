...
// Supabase Storage signed URLs (post images, brand assets) load cross-origin
// via <img>, so the browser returns OPAQUE responses. defaultCache's generic
// cross-origin route runs Serwist's copyResponse on them, which throws
// `cross-origin-copy-response` on an opaque body — the handler then synthesizes
// a 503, so EVERY post image fails (and the app shows the offline banner) even
// though the URL is fine (direct curl 200). Serve these straight from the
// network with no plugins and no copying. Signed URLs are unique per mint, so
// caching them would be pure waste anyway. MUST stay before `...defaultCache`.
