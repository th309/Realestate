...
export function SectionIcon({ sectionId }: { sectionId: string }) {
  ...
  if (id === "growth-potential-deep-dive")
  ...
  // V2 sections (shared across homeready_v2, investoredge_v2, comparison_v2)
  if (id === "executive-verdict") return <Sparkles className={iconClass} />;
  if (id === "market-deep-dive") return <BarChart3 className={iconClass} />;
  if (id === "your-situation") return <Users className={iconClass} />;
  if (id === "verdict-and-actions") return <Target className={iconClass} />;
  if (id === "what-to-watch") return <Activity className={iconClass} />;
  if (id === "investment-deep-dive") return <BarChart3 className={iconClass} />;
  if (id === "risk-and-resilience") return <Shield className={iconClass} />;
  if (id === "actions-and-monitoring") return <Target className={iconClass} />;
  if (id === "scenario-analysis") return <Layers className={iconClass} />;

  // Legacy / other report types
  if (id.includes("executive") || id.includes("summary"))
    return <FileText className={iconClass} />;
  if (id.includes("score") || id.includes("thesis"))
    return <TrendingUp className={iconClass} />;
  if (id.includes("afford")) return <DollarSign className={iconClass} />;
  if (id.includes("market") || id.includes("conditions"))
    return <BarChart3 className={iconClass} />;
  if (id.includes("risk")) return <AlertTriangle className={iconClass} />;
  if (id.includes("next") || id.includes("steps"))
    return <Activity className={iconClass} />;
  if (id.includes("cash") || id.includes("flow"))
    return <DollarSign className={iconClass} />;
  if (id.includes("appreciation") || id.includes("growth"))
    return <TrendingUp className={iconClass} />;
  if (id.includes("catalyst")) return <Sparkles className={iconClass} />;
  if (id.includes("price") || id.includes("trend"))
    return <BarChart3 className={iconClass} />;
  if (id.includes("supply") || id.includes("demand"))
    return <Activity className={iconClass} />;
  if (id.includes("talking") || id.includes("point"))
    return <MessageSquare className={iconClass} />;
...
export const SECTION_DISPLAY_NAMES: Record<string, string> = {
  ...
  "verdict-and-actions": "Verdict & Actions",
  "what-to-watch": "What to Watch",
  "investment-deep-dive": "Investment Deep Dive",
  "risk-and-resilience": "Risk & Resilience",
  "actions-and-monitoring": "Actions & Monitoring",
