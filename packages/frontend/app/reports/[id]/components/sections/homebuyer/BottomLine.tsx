...
/**
 * Derive the verdict type and label from the HomeReady score.
 */
...
/**
 * BottomLine - The actionable synthesis: what to DO.
 *
...
export function BottomLine({
  report,
  className = '',
}: BottomLineProps): React.ReactElement {
  const { verdict, label } = getVerdict(report.homeready_score);

  // ---- AI narratives ----
  const summaryRaw =
    (report.ai_narrative as any)?.bottom_line_narrative ??
    (report.ai_narratives as any)?.bottom_line_narrative ??
    (report.ai_narrative as any)?.bottom_line ??
    (report.ai_narratives as any)?.bottom_line ??
    null;
  ...
  // Action items
  const actionsRaw =
    (report.ai_narrative as any)?.bottom_line_actions ??
    (report.ai_narratives as any)?.bottom_line_actions ??
    null;
  const actionItems = parseArrayField(actionsRaw);

  // Watch items
  const watchRaw =
    (report.ai_narrative as any)?.bottom_line_watch ??
    (report.ai_narratives as any)?.bottom_line_watch ??
    null;
