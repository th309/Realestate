...
export class ConversionAnalyticsService {
  ...
  private async buildFullFunnel(
    ...
    for (const row of sessionRows || []) {
      sessionCountByVisitor[row.visitor_id] =
        (sessionCountByVisitor[row.visitor_id] || 0) + 1;
    }
    const activeVisitorCount = Object.values(sessionCountByVisitor).filter(
      (c) => c >= 2,
    ).length;
  ...
  private async buildPaywallEffectiveness(
    startDate: string,
    ...
    for (const evt of events || []) {
      ...
      if (evt.event_action === 'upgrade_complete')
        byResource[res].conversions++;
    }
  ...
  private async buildFeatureCorrelation(
    startDate: string,
  ): Promise<FeatureConvMetric[]> {
    const client = this.supabase.getClient();
    const [{ data: allSessions }, { data: conversionEvents }] =
      await Promise.all([
        client
          .from('user_sessions')
        ...
        client
          .from('user_events')
          .select('visitor_id')
          .eq('event_action', 'upgrade_complete')
          .gte('created_at', startDate),
      ]);

    const firstSessionByVisitor: Record<string, string> = {};
    for (const row of allSessions || []) {
      if (!firstSessionByVisitor[row.visitor_id])
        firstSessionByVisitor[row.visitor_id] = row.id;
    }
    ...
    const { data: featureEvents } = await client
      ...
      .in('session_id', firstSessionIds)
      .gte('created_at', startDate);

    const converters = new Set(
      (conversionEvents || []).map((r) => r.visitor_id),
    );
  ...
  private async buildRevenueMetrics(): Promise<{
    mrr: number;
    arpu: number;
