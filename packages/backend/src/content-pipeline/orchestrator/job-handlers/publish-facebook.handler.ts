 ...
 * with the visible URL before sending to Facebook. Mirrors TikTok and
 * YouTube Shorts publishers — same stored script, one source of truth
 * for what the placeholder resolves to at publish time.
 */
...
@Injectable()
export class PublishFacebookHandler {
  private readonly logger = new Logger(PublishFacebookHandler.name);

  constructor(
    private readonly supabase: SupabaseService,
    private readonly orchestrator: RunOrchestratorService,
    private readonly publisher: FacebookReelsPublisher,
  ) {}
  ...
  async handle(runId: string): Promise<void> {
    ...
    try {
      ...
      // Idempotent: clear any prior facebook_reels platform_post row so a
      ...
      this.logger.error(
        `[PIPE] publish-facebook FAILED run=${runId}: ${message.slice(0, 200)}`,
      );
      ...
      await client.from('content_run_events').insert({
        run_id: runId,
        event_type: 'facebook_publish_failed',
        payload: {
          message,
          code: e.code ?? null,
  ...
  private async downloadVideo(supabaseUrl: string): Promise<string> {
    ...
    const localPath = join(
      tmpdir(),
      `pub-fb-${randomBytes(4).toString('hex')}.mp4`,
    );
