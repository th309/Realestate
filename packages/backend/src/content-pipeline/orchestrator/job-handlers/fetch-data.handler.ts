...
export class FetchDataHandler {
  ...
  async handle(runId: string): Promise<void> {
    try {
      ...
      let augmentedSnapshot: Record<string, unknown> = snapshot as unknown as Record<string, unknown>;
      ...
      if (run.format === 'score_mover') {
        ...
        const ctx = await this.data.getScoreMoverContext(resolvedGeo.id, geo, windowDays);
        if (!ctx) {
          throw new Error(
        ...
        const existingScore = (augmentedSnapshot.score ?? {}) as Record<string, unknown>;
        augmentedSnapshot = {
          ...augmentedSnapshot,
          score: {
            ...existingScore,
