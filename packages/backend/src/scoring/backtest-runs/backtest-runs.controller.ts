 ...
 *
 * IMPORTANT: Route order matters in NestJS. Specific routes must be defined
 * BEFORE parameterized routes like :id to avoid incorrect matching.
 *
 * Endpoints (in definition order):
 ...
 * - GET /api/admin/backtest-runs/job/:jobId/status - Get job status
...
export class BacktestRunsController {
   ...
   * IMPORTANT: This route must be defined BEFORE :id routes to avoid being captured as an ID.
   */
  @Get('statistics')
  async getStatistics() {
    try {
   ...
   * to avoid capturing route segments like 'statistics' as an ID.
   */
  @Get(':id')
