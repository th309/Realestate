...
import {
  REALTOR_DATASETS,
  RealtorDatasetConfig,
} from '../config/realtor.config';
...
export class RealtorService {
      ...
      switch (config.geography) {
        ...
        case 'metro':
          ...
          if (hotnessMap.size > 0)
            records = this.mergeHotnessData(records, hotnessMap, 'cbsa_code');
          break;
        case 'county':
          records = this.parseCountyCoreCSV(coreCsv);
          if (hotnessMap.size > 0)
            records = this.mergeHotnessData(records, hotnessMap, 'county_fips');
          break;
        case 'zip':
          records = this.parseZipCoreCSV(coreCsv);
          if (hotnessMap.size > 0)
            records = this.mergeHotnessData(records, hotnessMap, 'postal_code');
          break;
        default:
  ...
  private mapCommonFields(row: any) {
    return {
      ...
      median_listing_price_per_square_foot: this.parseNumeric(
        row.median_listing_price_per_square_foot,
      ),
      median_listing_price_per_square_foot_mm: this.parseNumeric(
        row.median_listing_price_per_square_foot_mm,
      ),
      median_listing_price_per_square_foot_yy: this.parseNumeric(
  ...
  private parseHotnessData(
    ...
    for (const row of records) {
      ...
      map.set(key, {
        household_rank:
          this.parseInteger(row.hh_rank) ||
          this.parseInteger(row.household_rank), // field name varies?
        ...
        median_listing_price_vs_us: this.parseNumeric(
          row.median_listing_price_vs_us,
        ),
        page_view_count_per_property_mm: this.parseNumeric(
          row.page_view_count_per_property_mm,
        ),
        page_view_count_per_property_yy: this.parseNumeric(
          row.page_view_count_per_property_yy,
        ),
        page_view_count_per_property_vs_us: this.parseNumeric(
  ...
  private async insertRecords(
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      if (error) {
        this.logger.error(
          `Batch insert error for ${tableName}: ${error.message}`,
        );
        errors += batch.length;
      } else {
    ...
    return {
      ...
      errors:
        errors > 0 ? [{ message: `${errors} records failed to insert` }] : [],
    };
  }
}
