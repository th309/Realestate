...
export class RedfinService {
  ...
  async importRedfinData(
    ...
    // returning any for now to match structure, usually ImportResult
    ...
    if (!csvData) {
      try {
        csvData = await this.puppeteerService.downloadRedfinCSV(
          metricName,
          downloadUrl,
        );
      } catch (error: any) {
    ...
    for (const record of records) {
      try {
        ...
        if (!regionId) {
          ...
          regionId =
            (await this.geoMappingService.mapZillowRegionToGeoCode(
              regionName,
              '',
              regionType,
            )) || undefined;
    ...
    const overallStatus =
      errors === 0 ? 'success' : timeSeriesInserted > 0 ? 'partial' : 'failed';
    await reportPipelineStatus(
      'redfin',
      overallStatus,
      timeSeriesInserted,
