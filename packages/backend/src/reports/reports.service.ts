...
export class ReportsService {
  ...
  // ============================================================================
  // Per-Section Narrative Generation (v2)
  // ============================================================================

  /**
   * Generate section-specific AI narratives using structured prompt templates.
   *
   * This method replaces the flat dictionary approach with per-section prompts
   * that reference score components, benchmarks, and user inputs. Each section
   * gets a tailored prompt that produces grounded, data-specific content.
   *
   * @param reportData - The assembled report data (populated_data shape)
   * @param scores - Full score result with component breakdowns
   * @param dto - The original report generation request (for user inputs/priorities)
   * @param newsContext - Optional formatted news context string
   * @param sectionFilter - Optional list of section IDs to generate (defaults to all for report type)
   * @returns Record of section ID to generated narrative content
   */
  async generateSectionNarratives(
    ...
    scores: any, // ScoreResult from scoring service
    dto: GenerateReportDto,
    newsContext?: string,
    sectionFilter?: string[],
  ): Promise<Record<string, string | string[] | object>> {
    // 1. Build the context object from all available data
    const context = this.buildNarrativeContext(reportData, scores, dto, newsContext);
    ...
    if (sectionFilter && sectionFilter.length > 0) {
      ...
      const isComparison = dto.comparison_geographies && dto.comparison_geographies.length > 0;
    ...
    const sections = sectionIds
      ...
      .map((id) => {
        ...
        const resolvedTemplate = this.resolveConditionalBlocks(
    ...
    const results = await this.claudeService.generateNarratives(sections, context);
  ...
  buildNarrativeContext(
    ...
    if (hrComponents) {
      ...
      for (const comp of ['affordability', 'market_timing', 'stability', 'growth_potential']) {
    ...
    if (ieComponents) {
      ...
      for (const comp of ['cash_flow', 'rent_demand', 'appreciation', 'entry_point', 'risk']) {
        context[`${comp}_score`] = 'N/A';
        context[`${comp}_status`] = 'N/A';
        context[`${comp}_weight`] = 'N/A';
      }
    ...
    // Price metrics
    context.median_listing_price = this.formatCurrency(
      current.median_listing_price || current.zhvi,
    );
    context.zhvi = this.formatCurrency(current.zhvi);
    ...
    // Rent metrics
    context.zori = this.formatCurrency(current.zori);
    ...
    context.rent_to_price_ratio = current.rent_to_price_ratio?.toFixed(3) ?? 'N/A';
    ...
    context.income_needed_to_buy = current.income_needed_to_buy
      ? this.formatCurrency(current.income_needed_to_buy)
      : 'N/A';
    context.rent_to_income_ratio = current.rent_to_income_ratio?.toFixed(1) ?? 'N/A';

    // Economic/Census data
    context.median_income = this.formatCurrency(
      current.median_income || current.median_household_income,
    );
    context.median_household_income = this.formatCurrency(
      current.median_household_income || current.median_income,
    );
    context.population = current.population?.toLocaleString() ?? 'N/A';
    context.population_growth_yoy = current.population_growth_yoy?.toFixed(1) ?? 'N/A';
    ...
    context.zhvi_vs_2012_trough = current.zhvi_vs_2012_trough?.toFixed(1) ?? 'N/A';
    ...
    context.national_median_price = this.formatCurrency(benchmarks.national_median_price);
    context.state_median_price = this.formatCurrency(benchmarks.state_median_price);
    context.national_avg_cap_rate = benchmarks.national_avg_cap_rate?.toFixed(2) ?? 'N/A';
    ...
    if (historical.days_on_market) {
      ...
      context.dom_change_pct = historical.days_on_market.change_pct?.toFixed(1) ?? '0';
    } else {
    ...
    if (historical.cap_rate) {
      ...
      context.cap_rate_change_pct = historical.cap_rate.change_pct?.toFixed(1) ?? '0';
    } else {
    ...
    context.user_income = rawIncome != null ? this.formatCurrency(Number(rawIncome)) : null;
    context.user_down_payment =
      rawDownPayment != null ? this.formatCurrency(Number(rawDownPayment)) : null;
    context.user_budget = rawBudget != null ? this.formatCurrency(Number(rawBudget)) : null;
    context.user_timeline = userInputs.timeline || null;
    context.user_target_return = userInputs.target_return || null;
    context.user_strategy = userInputs.strategy || userInputs.investment_strategy || null;

    // -----------------------------------------------------------------------
    // News Context
    // -----------------------------------------------------------------------
    context.news_context = newsContext || 'No recent news available for this market.';
    ...
    if (reportData.comparisons && Object.keys(reportData.comparisons).length > 0) {
      ...
      const compComponentParts: string[] = [];
      ...
      for (const compName of componentNames) {
        ...
        for (const [compId, compData] of compEntries) {
          const compGeo = (compData as any).geography;
          const compScores = (compData as any).scores;
          const compComponents = compScores?.scores?.[scoreType]?.components || [];
      ...
      if (priorityWeighted) {
        ...
        const priorityLines = (priorityWeighted.priorityScores || []).map(
          (ps: any) =>
            `- Priority "${this.formatPriorityName(ps.priority)}" (weight ${ps.weight}x): Winner = ${ps.winnerName} — ${ps.reason}`,
        );
  ...
  async regenerateNarratives(
    ...
    if (updateError) {
      this.logger.error(`Failed to update user inputs: ${updateError.message}`);
    }

    // 4. For now, return the keys that would be regenerated
    // Full AI regeneration will be wired when the narrative service supports selective regeneration
   ...
   *
   * This is used internally by buildNarrativeContext is NOT needed since
   * ClaudeService.interpolateTemplate already handles {{variable}} substitution.
   * However, the conditional blocks need pre-processing before interpolation.
   *
   ...
   */
  private resolveConditionalBlocks(
    template: string,
