...
const PROMPTS: Record<SectionId, string> = {
  ...
  recommendation_analysis:
    ...
    ' If the 30-YEAR WEALTH PROJECTION block is present and its long-term trajectory diverges sharply from the year-one economics, add one sentence naming the divergence: cite the final equity or cumulative cash flow figure, name the growth assumption doing the work (rent growth or appreciation), and remind the investor that the grade reflects year-one economics while that long-term outcome only materializes if the assumption holds. When the growth assumption is aggressive, say plainly that the projection leans on it rather than presenting the outcome as a given.' +
    ' CRITICAL FRAMING: the STRATEGY block names the play being analyzed, and that is what this analysis IS. Your opening sentence must be framed by that strategy. Never open by asserting a goal, a holding period, or a payoff timeline that the strategy does not actually produce: a buy and hold analysis must never begin "your goal is fast cash within 12 months", because buy and hold is not a 12-month play. If no STRATEGY block is present, stay neutral rather than guessing at one.' +
    " If a USER GOAL block is provided (one of: cash flow, long-term wealth, fast cash, recycle capital), treat it as a LENS on the strategy above, never as a replacement for it. Judge the deal against that goal's primary metric: cash flow → monthly cashflow and cash-on-cash return; long-term wealth → 30-year wealth projection and equity buildup; fast cash → net profit and time-to-cash; recycle capital → cash left in the deal after refinance and how quickly the original down payment comes back. The lever you recommend should be the one that most moves the needle for THIS goal, not a generic grade-lift." +
    ' When the goal and the strategy pull in the same direction, name both together in the first sentence (for example "as a buy and hold aimed at monthly cash flow, this one grades a B"). When they PULL APART, do not paper over it and do not restate the goal as though it were the plan: say plainly that the deal is being analyzed as the named strategy, that the stated goal wants something different, and what that costs. For example, a buy and hold held for the long term cannot deliver a lump sum inside 12 months, so if the goal is fast cash, say the strategy and the goal are mismatched and name the strategy that would actually serve the goal. If the grade is decent but the deal does not serve the stated goal (a B-grade buy and hold throwing off $80 a month when the goal is maximum cash flow), say so directly and recommend a different deal shape or a different strategy.',
  projection:
