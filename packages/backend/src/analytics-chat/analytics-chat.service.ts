...
USER QUERY:`;
  ...
  async * chatStream(
    ...
    // Check for pre-computed briefing and cached rankings to enrich the prompt.
    const briefingResult = await this.lookupBriefingContext(userMessage, conversation.context);
    const rankingsContext = await this.lookupRankingsCache(userMessage);

    if (briefingResult) {
      const { briefing, freshNews } = briefingResult;
      systemPrompt += this.formatBriefingForPrompt(briefing, freshNews);
      this.logger.log(`[Quinn Stream Intelligence] Injected briefing for ${briefing.geography_name}`);
    }
    if (rankingsContext) {
      systemPrompt += `\n\n${rankingsContext}`;
      this.logger.log(`[Quinn Stream Intelligence] Injected cached rankings context`);
    }
  ...
  async chat(
    ...
    // Check for pre-computed briefing and cached rankings to enrich the prompt.
    // These are additive — if lookups fail or return null, the existing flow continues.
    const briefingResult = await this.lookupBriefingContext(userMessage, conversation.context);
    const rankingsContext = await this.lookupRankingsCache(userMessage);

    if (briefingResult) {
      const { briefing, freshNews } = briefingResult;
      const briefingContextStr = this.formatBriefingForPrompt(briefing, freshNews);
      systemPrompt += briefingContextStr;
      this.logger.log(`[Quinn Intelligence] Injected briefing for ${briefing.geography_name}`);
    }
    if (rankingsContext) {
      systemPrompt += `\n\n${rankingsContext}`;
  ...
  // ===========================================================================
  // Market Intelligence: Briefing & Rankings Pre-Lookup
  // ===========================================================================
  ...
  private async lookupBriefingContext(
    ...
    try {
      ...
      this.logger.warn(`Briefing lookup failed: ${error.message}`);
   ...
   */
  private async lookupRankingsCache(message: string): Promise<string | null> {
    try {
      ...
      const rankings = await this.rankingsCache.getRanking(metricId, 'metro', direction);
      ...
      this.logger.warn(`Rankings cache lookup failed: ${error.message}`);
      return null;
    }
  }
