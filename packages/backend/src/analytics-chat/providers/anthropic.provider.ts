...
export class AnthropicProvider implements AIProvider {
  ...
  async *chatStream(options: ChatOptions): AsyncGenerator<ChatStreamChunk> {
    ...
    while (
      ...
      this.logger.log(
        `[AnthropicProvider] Processing ${toolUseBlocks.length} tool calls`,
      );
      ...
      for (const toolUse of toolUseBlocks) {
        ...
        yield {
          type: 'tool',
          content: { name: toolUse.name, status: 'executing' },
        };
        ...
        yield {
          type: 'tool',
          content: { name: toolUse.name, status: 'complete' },
        };
      }
  ...
  async chat(options: ChatOptions): Promise<ChatResponse> {
    ...
    while (response.stop_reason === 'tool_use' && iterations < maxIterations) {
      ...
      for (const toolUse of toolUseBlocks) {
        ...
        this.logger.log(
          `[AnthropicProvider] Executing tool: ${toolUse.name} with input: ${JSON.stringify(toolUse.input)}`,
        );
        let result: any;
        try {
          result = await this.toolsService.executeTool(
        ...
        this.logger.debug(
          `[AnthropicProvider] Tool ${toolUse.name} result: ${JSON.stringify(result).slice(0, 500)}`,
        );
      ...
      const nextText = response.content
