...
export class OpenAIProvider implements AIProvider {
  ...
  async *chatStream(options: ChatOptions): AsyncGenerator<ChatStreamChunk> {
    ...
    const apiMessages = [
      {
        role: 'system',
        content: `${QUINN_DEEPSEEK_SYSTEM_PROMPT}\n\n${systemPrompt}`,
      },
      ...messages.map((m) => ({
        role: m.role,
        content: m.content,
    ...
    const currentContextMessages: any[] = [...apiMessages];
    ...
    while (runLoop && iterations < maxIterations) {
      ...
      // Process Tool Calls
      this.logger.log(
        `[OpenAIProvider] Processing ${toolCalls.length} tool calls (Iteration ${iterations})`,
      );
      ...
      for (const tc of toolCalls) {
        ...
        try {
          ...
          this.logger.error(`Failed to parse args for ${name}`);
        }
  ...
  async chat(options: ChatOptions): Promise<ChatResponse> {
    ...
    while (iterations < maxIterations) {
      ...
      for (const tc of msg.tool_calls as any[]) {
        this.logger.log(
          `[OpenAIProvider] Executing tool: ${tc.function.name} with args: ${tc.function.arguments}`,
        );
        ...
        try {
          ...
          this.logger.error('Args parse error');
        }
        ...
        if (!result) {
          result = {
            success: false,
            error: 'Tool execution failed silently (null result)',
          };
        }

        this.logger.debug(
          `[OpenAIProvider] Tool ${tc.function.name} result data keys: ${JSON.stringify(Object.keys(result?.data || {}))}`,
        );
    ...
    this.logger.log(
      `[OpenAIProvider] Chat loop finished. Total tools used: ${toolsUsed.length}`,
    );
