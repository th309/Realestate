...
| Variable | Required | Description |
|---|---|---|
| `ANTHROPIC_API_KEY` | ✅ | Used by Quinn chatbot and narrative generation |
| `AI_PROVIDER` | ⚠️ Optional | `anthropic` (default) or `openai` — controls Quinn model provider |
...
| `ANALYTICS_TOOLS_ENABLED` | ⚠️ Optional | Set to `true` to enable experimental analytics tools for Quinn |
| `QUINN_CACHE_WARM_ON_STARTUP` | ⚠️ Optional | Set to `true` to pre-warm Quinn cache on boot |
