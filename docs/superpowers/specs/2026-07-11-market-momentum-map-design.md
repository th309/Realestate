...
- Click / Enter: navigate (Next `Link`-equivalent behavior) to `/markets/metro/[slug]`, slug built with the same helper the markets pages use. Metros without a live market page are non-clickable but still hoverable.
