...
| MoS computation formula          | v1: `active_listings / nullif(homes_sold, 0)` aggregated per (region, period*end). **KNOWN BIAS (measured 2026-05-23):** computed runs ~20-50% higher than legacy MoS (e.g. CBSA 16980 computed 4.20 vs legacy 2.90; 38060 5.45 vs 3.60). Same magnitude/ranking but a systematic upward bias vs Redfin's official methodology. **MUST refine before the fallback goes live** (i.e. before legacy `redfin*\*` MoS is deprecated — the Option-C window). Refinement options: 3-month rolling homes_sold; reconcile new-format ACTIVE LISTINGS vs legacy INVENTORY definitions. Fallback is currently DORMANT (legacy present → merge is a no-op), so this bias does not affect the live score today. | Redfin's official MoS uses a smoothed sales pace + its own inventory measure; point-in-time active/homes diverges.                                                       |
