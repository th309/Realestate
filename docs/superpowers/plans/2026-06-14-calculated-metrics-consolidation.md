...
**C. DONE** (commit `75a85f7a`) — chose option (b): kept the runner + affordability scripts for the local realtor-import flow (the parallel session's `60aaa8f7` repoints `import-realtor-data.ts` at the runner). Deleted only the truly-unreferenced duplicates: `calculations/investment-metrics.ts`, `calculations/valuation-metrics.ts`, the `utils/refresh-calculated-metrics.ts` shim, `run-refresh-calculated.ts`, `refresh-all-metrics.ts`, `ensure-calculated-metrics-populated.ts`. Repointed `import-all-non-zillow.ts` → runner (it had been calling the already-broken `refresh-all-metrics.ts`, which read `RefreshResult` fields the trimmed runner no longer returns — so this also fixed a latent runtime throw). Fixed two stale help strings. Verified: no dangling references to any deleted file.

**REMAINING: E only (DEFERRED to a fresh session, per user).**

- **E** is a large ATOMIC refactor (now ~3,929 lines incl. affordability): extract investment + valuation (+ affordability) calculators behind the facade, preserve exact behavior (MOS latest-period gating, overvalued non-clobber + 1000-row pagination, absorption_rate, HUD fallbacks), then re-run the refresh CLI + re-check coverage. Deferred deliberately — needs full context budget + careful live re-verification, not the tail of a long session.
