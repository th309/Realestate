...
| Table                       | Purpose                                                                        |
| --------------------------- | ------------------------------------------------------------------------------ |
| `propertyiq_scores`         | Current PropertyIQ Score for all geographies (single score type: 'propertyiq') |
...
### PropertyIQ Score (Single Unified Score)

> **NOTE:** The legacy 3-score system (HomeReady, InvestorEdge, MarketHealth) was replaced by a single PropertyIQ Score in March 2026. See `docs/superpowers/specs/2026-03-29-propertyiq-single-score-redesign.md` for the full specification.

**Purpose:** Measure market demand signal relative to state average. Score of 50 = state average; higher = outperformance.
...
| Input Metric              | Direction            | Source | Definition                                                              |
| ------------------------- | -------------------- | ------ | ----------------------------------------------------------------------- |
| **% Sold Above List**     | Higher is better (+) | Redfin | Percentage of homes selling above list price. Measures demand strength. |
| **Median Days on Market** | Lower is better (-)  | Redfin | Median days from listing to pending. Faster sales = stronger demand.    |
| **Months of Supply**      | Lower is better (-)  | Redfin | Active inventory / monthly sales pace. Lower = tighter market.          |

**Key properties:**

- Scores are relative within each state, not nationally
- 50 = state average performance
- Stored in `propertyiq_scores` table with `score_type = 'propertyiq'`
...
| Score | Label     | Meaning                                |
| ----- | --------- | -------------------------------------- |
| 90+   | EXCELLENT | Top-performing market within its state |
| 80-89 | GREAT     | Strong demand signal vs state peers    |
| 70-79 | GOOD      | Above-average demand                   |
| 60-69 | FAIR      | Slightly above average                 |
| 50-59 | AVERAGE   | In line with state median              |
| 40-49 | BELOW AVG | Below state average demand             |
| 20-39 | POOR      | Weak demand signal                     |
| <20   | VERY POOR | Significantly underperforming state    |
...
| -------------------- | --------------------------------------- | ----------- |
| **PropertyIQ Score** | Score visible, breakdown details locked | Full access |

**Geographic Coverage:** The PropertyIQ Score works identically at every geographic level (Metro, County, ZIP). Users see the same score experience regardless of which level they select.
...
_Note: Market Health only validates 6m and 1y horizons — it predicts short-term conditions, not long-term trends._
...
_Last updated: January 2026 (PropertyIQ Scores v2.0 with data inheritance)_
