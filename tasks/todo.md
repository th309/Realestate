...
- [~] **T2** #2 Three distinct persona finales — BACKEND DONE: per-persona narrative `systemPromptFor(persona)` (homebuyer/investor/agent intros + shared rules), fallback made persona-neutral; backend tsc clean. FRONTEND TODO: 3 distinct finale components (HomebuyerFinale/InvestorFinale/AgentFinale) + route `Step4Aha` by `session.persona`. Verify each persona end-to-end.
