...
## Verify a Probe Route Exists in Code Before Interpreting Its 404

**Date:** 2026-07-27
**Context:** Debugging why scheduled social posts never published, I probed prod `GET /api/admin/social-connect` and got 404 while a sibling admin route returned 401, and concluded the SocialConnectModule wasn't in the deployed build ("stale deploy since July 25"). Wrong: the controller has no root GET — its list route is `@Get('connections')`. Probing the real path `/api/admin/social-connect/connections` returned 401, proving the module was deployed all along. The actual root cause was a missing `LATE_API_KEY` env var on the Railway backend, whose absence makes the publisher no-op silently every minute (it logs a single warning per process at boot, then nothing).

**Rule:** A 404-based deduction is only valid if you have verified in the controller source that the exact probed path+method exists. Before concluding anything from a 404, read the `@Controller`/`@Get`/`@Post` decorators and construct the full route from them; use a sibling known-good route only as the auth-behavior control (401 vs 404), not as proof your guessed path should exist. More broadly: when a probe result would flip a conclusion (deployed vs not deployed), validate the instrument before trusting the reading.

## Piping a Build Through `tail` Reports the Pipe's Exit Code, Not the Build's

**Date:** 2026-07-28
**Context:** Verifying the signup-conversion fix, I ran `npx next build 2>&1 | tail -35` in the background. The harness reported "exit code 0" and the visible tail showed only stack-trace lines, so I read it as a passing build. It was not: re-running unpiped as `npx next build > /tmp/build.log 2>&1; echo "REAL_EXIT=$?"` gave `REAL_EXIT=1` with 10 `Module not found` errors. In a shell pipeline the `$?` is the LAST command's status — `tail` succeeds essentially always, so it launders any non-zero exit from the real command into a green result. The `| tail -N` also truncated the log file itself, so the later greps for "Compiled successfully" and "Module not found" both came back empty and looked like confirmation of success when they were actually confirmation of nothing.

**Rule:** Never determine pass/fail of a build, test run, typecheck, or migration from a command whose output is piped. Redirect the FULL output to a file and capture the real status separately — `cmd > log 2>&1; echo "EXIT=$?"` — then grep the complete file, or in bash read `${PIPESTATUS[0]}` when a pipe is unavoidable. An empty grep result over a truncated log is not evidence of anything. Corollary for verification generally: before treating a signal as proof, confirm the instrument actually measured what you think — the same failure mode as "verify a probe route exists before interpreting its 404" above.

## Invoke the Process Skills at the Design/Plan Transition, Not Just at the Bug

**Date:** 2026-07-29
**Context:** Asked why bot-exclusion work wasn't visible on `/admin/analytics`, I correctly invoked `superpowers:systematic-debugging` and used it well — root cause found with real evidence. But the task then crossed a boundary from *debugging* into *designing and planning* a multi-phase change, and I never re-checked for applicable skills at that transition. I wrote a 7-phase plan with acceptance gates without invoking `superpowers:brainstorming` or `superpowers:writing-plans`, and skipped plan mode despite §2.1 requiring it for anything with 3+ steps or architectural decisions. Troy had to ask "why aren't you using your skills as you should be in claude.md." Separately, §1.5 (default to parallel agents) conflicted with a session-level instruction not to use the Agent tool; I silently resolved it toward the session instruction, which read as unexplained non-compliance rather than a deliberate call.

**Rule:** Skill applicability is evaluated per *phase of work*, not once per conversation. When the work changes character — debugging → designing, designing → planning, planning → implementing — re-run the skill check before the first action of the new phase. The concrete transitions in this repo: starting any creative/design work → `superpowers:brainstorming`; having requirements for a multi-step task → `superpowers:writing-plans` + plan mode per §2.1; writing any production code → `superpowers:test-driven-development`; claiming completion → `superpowers:verification-before-completion`. "I already invoked a skill this session" is not a reason to skip the next one — systematic-debugging says nothing about how to design a schema migration.

**Corollary on conflicts:** when a CLAUDE.md rule and a session-level instruction genuinely conflict (here: §1.5 parallel agents vs. "do not call the Agent tool unless requested"), say so out loud and state which one is being followed and why. Silently picking one makes correct behavior indistinguishable from ignoring the project's rules — and denies the user the chance to resolve the conflict.
